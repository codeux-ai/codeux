...
- `tests/e2e/navigation/**/*.spec.ts`, which verifies the local dashboard shell, route landmarks, sprint ledger navigation, responsive navigation behavior, accessibility smoke coverage, speed audits, product smoke paths, release-path app smoke coverage, and Docs page smoke coverage for `/docs`, the docs overview, dashboard overview, and representative sample pages.
- `tests/e2e/tasks/**/*.spec.ts`, which verifies draft task creation and editing, task deletion, long task markdown round-trips, dependency preservation across reload/edit flows, sprint draft CRUD, sprint/task lifecycle flows, visual composer feedback, deterministic fake-provider invocation runtime coverage, full sprint orchestration, and sprint controls stress behavior.
- `tests/e2e/projects/**/*.spec.ts`, which verifies first-run project setup coverage, local project CRUD, selected-project changes, duplicate and missing local path handling, and host filesystem persistence, including instruction-file writes, local-directory and local-file browsing, sanitized traversal rejection, nested OS-temp fixtures, Windows-compatible path construction, and Settings Appearance background-image uploads. The Playwright server exposes the OS temp directory through `CODE_UX_DIRECTORY_BROWSER_ROOTS` so temporary git fixtures can be browsed without Docker-backed file-browser sessions.
- `tests/e2e/agents/**/*.spec.ts`, which verifies agent avatar and agent surface browser coverage.
- `tests/e2e/settings/**/*.spec.ts` and `tests/e2e/config/**/*.spec.ts`, which verify Settings and Config workflows, including project override persistence, inherited effective settings, provider/runtime controls, invalid input feedback, disabled inherited controls, and advanced MCP/integration/memory/automation persistence without real provider invocation.
...
- For CRUD browser coverage, drive the visible composer or modal for the workflow being tested, then use exported API helpers only for deterministic setup, polling, and cleanup. Reload after save when persistence is part of the contract, and assert dependency arrays or effective settings through public HTTP APIs.
