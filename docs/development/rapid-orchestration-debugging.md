...
| Merge-conflict attention opens and is immediately dismissed while the task still reports conflict in the dashboard. | Resolved `project_attention_items.payload_json.branchMergeRetryConsumed`, task `merge_indicator`, runtime status sync logs. | A consumed virtual-worker resolution is still suppressing the fresh `MERGE_CONFLICT`; suppression must ignore resolved conflict rows after their retry has been consumed. |
