...
- sprint status badges do not infer human escalation from that bounded queue. The compact `sprintWorkflowProjections` read model scans durable active attention rows and only aggregates task-level `open`/`claimed` items owned by `human`/`user` with no assigned worker endpoint. This preserves real human-needed state beyond the 50-item activity feed while excluding work that a worker owns or has already claimed.
