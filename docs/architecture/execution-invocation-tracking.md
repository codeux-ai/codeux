...
Invocation persistence sanitizes provider output before chat-facing invocation message content is written. It redacts secret-shaped values such as provider API keys, Git host tokens, authorization headers, URL credentials, and provider auth path fields; it also removes known non-actionable bootstrap noise such as `fatal: your current branch 'code-ux-bootstrap-*' does not have any commits yet` and the legacy Qwen OpenAI logging warning. Other failure text remains visible after redaction so real provider failures still surface.
