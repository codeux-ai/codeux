...
During live telemetry, `ProviderExecutionService` rewrites invocation messages from parsed provider conversation turns for every provider invocation purpose that carries an agent transcript, including planning, QA, remediation, setup, chat replies, and task coding. The rewrite signature is derived from the persisted message payloads so changes in reasoning text, tool arguments, tool output, or metadata trigger a refresh even when the transcript length does not change. When a provider only exposes final text, the service falls back to appending the sanitized assistant text instead of replacing earlier retry/history messages.
...
The dashboard Chat -> Invocations rail renders only `execution_invocations` returned by the server list endpoint or project realtime refreshes. Sending a chat message still updates the thread transcript from the returned conversation message immediately, but the invocation rail waits for the persisted backend invocation row instead of inserting a client-only placeholder.
