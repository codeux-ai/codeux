...
Invocation persistence sanitizes provider output before chat-facing invocation message content is written. It redacts secret-shaped values such as provider API keys, Git host tokens, authorization headers, URL credentials, and provider auth path fields. It also applies targeted cleanup for noisy cases, including removing the bootstrap branch-no-commits line (`fatal: your current branch 'code-ux-bootstrap-*' does not have any commits yet`) and the legacy Qwen OpenAI logging warning. User prompt content bypasses this sanitizer and is stored exactly. All other `fatal:` lines and unrelated stderr/stdout remain unchanged so real provider failures still surface after redaction. Other failure text remains visible after sanitization so providers can still surface actionable diagnostic context.

CLI provider telemetry keeps raw parser inputs inside the runner long enough for Codex, Claude Code, Qwen Code, OpenCode, Gemini, and Antigravity parsers to reconstruct usage and conversations, but callback-facing telemetry and final runner results are sanitized before they can be persisted into `execution_invocation_messages`, activity streams, or structured invocation logs. Watcher read failures are purpose-labelled as `runtime`, provider invocation lifecycle logs are purpose-labelled as `invocation`, and repeated watcher read failures warn once for a failing polling path instead of emitting an unbounded warning stream.
