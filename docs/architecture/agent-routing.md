...
Unset manual routes fall back to the existing built-in preset for that route. Coding, CI fix, merge conflict, and dashboard replies use `Worker`; clarification replies use `Project manager`.
...
Manual coding routing omits the roster and keeps the existing task schema without requiring task-level agent assignments.
