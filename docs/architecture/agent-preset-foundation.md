...
Dashboard chat replies are the only default exception. Each assigned dashboard reply agent receives the full built-in Code UX MCP surface, the restricted `scheduler_code_ux` tool, and the default Playwright MCP server by default, even when the selected reply preset has Code UX disabled or no saved MCP policy. Runtime dispatch preserves the selected agent's linked custom MCP servers, adds the Playwright link once, and sends the assigned agent id through the built-in `code_ux` connection, allowing the MCP router to apply the dashboard-reply full-access default. Planning, coding, CI fix, merge-conflict, clarification, QA, generated setup, and general Project manager agent runs remain denied unless their preset explicitly enables Code UX tools; when Code UX is first enabled for those non-dashboard agents, `scheduler_code_ux` is explicitly off by default.
