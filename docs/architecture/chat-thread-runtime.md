...
### Cinematic Invocation Feedback Projection

3D Chat derives foreground invocation feedback from persisted execution data; it is not a second conversation stream. `useCinematicInvocationFeedback` considers only invocations in the selected project, then selects the newest running `dashboard_reply` or `worker_reply` whose `agentPresetId` matches the Project Manager preset resolved for the stage. Start time orders candidates and invocation id breaks ties deterministically. Invocation-pane selection has no effect on this ownership. Other running replies, task coding, planning, QA, and CI work remain available to the project-wide activity resolver as a background cue and count, but cannot equip a Project Manager work tool or create its transient progress bubble. An awaited thread reply may still drive the general thinking state, but without a matching invocation it does not drive transcript feedback or a runtime-selected tool.

For the selected invocation, the hook fetches the persisted invocation transcript even when `messageCount` is zero. `projectCinematicInvocationFeedback` exposes only the latest non-empty `role: "assistant"` turn whose normalized `metadata.kind` is absent or `assistant`; it excludes user, system, reasoning, injected-context, tool, and unknown internal turns. Tool activity counts normalized `tool_call` and `tool_result` messages by `toolCallId`, so a call/result pair counts once, and falls back to the stable persisted message id when no call id exists. The resulting bubble always shows explicit **In progress** and tool-count text, plus either that safe interim prose or the zero-message startup placeholder. It is a frontend projection, not a durable `ConversationMessageRecord`: it stays separate from the thread transcript and is removed before the durable final reply is presented.

Transcript refresh is keyed by the selected invocation's `messageCount`, `lastMessageAt`, and `updatedAt`. This deliberately covers both appended turns and structured telemetry rewrites whose length is unchanged. While a same-invocation refresh is pending, the last accepted message and count remain available. A transcript-fetch failure is non-fatal and preserves that snapshot, or the startup placeholder if no successful snapshot exists. Each request has an abort signal and generation token; invocation/project changes and unmount abort pending work, and late out-of-order responses are ignored even if the transport completes after cancellation.

The snapshot and stage ownership clear when the invocation disappears, becomes terminal, is replaced, leaves the selected project, or no longer matches the resolved preset. The stage also binds foreground feedback to the project/thread context in which that invocation first appeared: the initial `new-thread` to newly created thread transition is retained for the first send, while later thread changes clear the bubble and work tool. These rules prevent stale interim text from following navigation or duplicating the stored final reply. They do not imply provider delivery guarantees or a backend streaming contract.

Presentation remains readable without animation. With motion enabled, the progress bubble animates on first appearance and eligible interim-message changes, while a matching invocation rotates through its deterministic tool catalog every seven seconds. Reduced motion suppresses those progress transitions, thinking dots, and tool rotation while leaving the initial static tool and all visible status, interim prose, startup, and count labels intact; WebGL failure uses the same static semantics. Compact layouts keep the latest exchange above the composer in a bounded scroller. At wider breakpoints the reply/progress column is capped at 680px on LG, 780px on XL, and 880px on 2XL; progress prose reaches 17px with 36px line height at 2XL, and the bubble remains height-bounded with internal vertical scrolling plus horizontal overflow for tables and code. The separate 208px desktop thought bubble shifts farther left at XL/2XL and preserves spacing around its two-dot tail so it does not collide with the enlarged avatar or reply column. Visible text, rather than motion or tool geometry, is the authoritative activity signal.
...
### Thread Single-Flight and Scheduled Continuations

`ChatThreadRuntimeService` owns at most one provider turn per thread through its in-memory `inFlightTurns` map. `isThreadBusy(threadId)` is a read-only guard over that ownership: scheduler due checks use it only for `agent_wakeup` entries that already identify an originating thread. A busy thread leaves the occurrence scheduled and due, without advancing recurrence or recording delivery. Wakeups without a thread id and targets for other threads keep their existing dispatch behavior. Reply finalization removes thread ownership before asking the scheduler to check due entries again, so a deferred occurrence can be accepted immediately after the active turn settles.

The runtime classifies messages carrying `metadata.source` or `metadata.origin` equal to `agent_scheduler` separately from ordinary dashboard messages. A scheduled message that reaches `postMessage` during a race with an active turn is persisted as pending but does not abort that turn. The owning loop drains pending messages in repository order (`createdAt`, then id), coalescing only adjacent messages of the same class. This gives multiple queued wakeups one ordered provider continuation while keeping scheduled and user-authored prompts from being merged into one semantic turn. Each drained batch uses the last persisted message as its status boundary, and an in-memory settled-id set prevents a completed or failed batch from being invoked twice during finalization.

Ordinary dashboard messages retain supersession behavior: a newer message aborts the provider attempt, leaves the interrupted messages pending, and combines the adjacent ordinary messages into the next continuation against the saved provider session. Explicit dashboard cancellation uses a distinct abort reason. It marks only the requested in-flight batch failed, posts no synthetic worker-failure reply, and never replays that cancelled batch. Provider failures remain observable through the existing structured error log, failed delivery transition, and single visible system failure reply; later queued batches continue through the same thread owner without overlapping provider sessions.
