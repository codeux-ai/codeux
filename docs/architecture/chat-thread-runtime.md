...
Create-app quickactions are the exception to the normal routed reply path after persistence. Dashboard thread controls post a short visible message with `metadata.quickaction.type = "create_app"`, canonical `kind` (`web_app` or `desktop_app`), a stable `requestId`, and the matching quicksprint template id. `ChatThreadRuntimeService.postMessage` stores that dashboard message first, validates the metadata, launches the detached quicksprint in `plan_and_start` mode, and posts an app-progress system widget without asking for confirmation or creating a dashboard reply invocation.
