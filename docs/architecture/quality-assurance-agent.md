...
- every task- and sprint-completion reviewer row records the exact review execution invocation, logical reviewer session, isolated workspace session, reviewer preset, and continuation provenance. The invocation is created before snapshot preparation so operators can observe that phase; if preparation fails before provider linkage, Code UX closes the invocation with the preparation failure rather than leaving a stale running audit row. Under the restart `continue` policy, a retry reuses that reviewer workspace and continues the provider's native session when available instead of starting the review again without its prior investigation context. Reuse verifies that the preserved Docker snapshot has a valid Git `HEAD`; if restart interrupted initialization after volume creation, Code UX rebuilds the snapshot from the requested review branch instead of reviewing an empty volume.
...
- QA continuation keeps logical provider-session identity separate from workspace identity. It resolves the latest compatible `cli_workspace_bound` record for the task, reuses that exact preserved path/session when available, and verifies the checked-out worker branch before dispatch. The coding invocation used for a native continuation is selected across both the logical and workspace-owner session ids, preferring the invocation linked to the workspace-binding task run. This keeps the provider thread and paired runtime home together for retries whose newer task session intentionally points at an older workspace without bypassing fresh-branch collision protection.
...
- startup recovery also reconciles stale `running` QA review rows and stale QA invocation audit rows globally. If the backing QA execution invocation already ended, never linked to provider runtime, or points at a Docker-backed provider invocation whose container is gone, startup cancels the QA run and backing invocation so the sprint can retry instead of keeping a historical `QA review running` badge indefinitely. Because startup is a process boundary, an invocation that never linked to provider runtime is cancelled immediately rather than receiving an in-process dispatch grace period.
...
- task-level QA runs are surfaced in task list records and live runtime snapshots. Tasks, task-oriented Live surfaces, and Overview project the latest review into the six-stage task delivery workflow badge. Sprint gallery and ledger surfaces project it into the seven-stage sprint workflow with Planning first and Pull request after QA. Both show an active QA stage while the latest task QA run is still `running`.
...
For the reviewer track, Code UX persists the exact execution invocation and workspace binding on the reviewer-specific QA row before dispatch. A runtime restart under the `continue` invocation policy closes the interrupted audit invocation, preserves the snapshot workspace, and starts a correlated continuation with the same logical session while finishing that partial review cycle. When the CLI provider supplied a native session id, Code UX passes it to the resumed invocation so the reviewer retains conversation context as well as filesystem state. After the cycle records a decisive verdict, any later verification uses a new branch snapshot and reviewer session so coding follow-up changes cannot be hidden by a pre-fix workspace. Multi-reviewer cycles recover each preset independently; timestamp proximity is only a compatibility fallback for older rows that predate exact invocation correlation.
