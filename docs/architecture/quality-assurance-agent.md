...
- QA continuation keeps logical provider-session identity separate from workspace identity. It resolves the latest compatible `cli_workspace_bound` record for the task, reuses that exact preserved path/session when available, and verifies the checked-out worker branch before dispatch. This covers retries whose newer task session intentionally points at an older host worktree without bypassing fresh-branch collision protection.
