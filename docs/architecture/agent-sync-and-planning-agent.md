...
6. The setup prompt includes the same selected project guidance before artifact instructions. If tech-stack guidance is selected but the styleguide remains `none`, the prompt tells the setup agent to inspect existing styling, brand assets, design tokens, components, layouts, and interaction patterns before proposing a repository-specific styleguide.
