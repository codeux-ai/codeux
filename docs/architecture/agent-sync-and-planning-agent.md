...
In Docker execution mode, planning runs against a snapshot workspace and captures `.task-learnings.md` back out of that snapshot volume so memory capture still works even though the provider never writes directly into the host repo path. In `REMOTE` git mode, fresh planning invocations refresh `origin` and resolve only the explicit sprint feature branch when present, otherwise the effective runtime git default branch. When a GitHub or GitLab credential reference is configured, planning resolves it through the credential broker with the active project and planning-snapshot workspace scope before refreshing remote refs or creating the snapshot. Each refresh resolves the reference again so rotation and revocation take effect immediately; malformed, missing, unavailable, revoked, or out-of-scope references fail before Git or workspace creation runs. Plaintext credentials remain bounded to the remote operation and are not written to invocation records, logs, telemetry, or errors. Local Git behavior and explicit environment-token compatibility remain unchanged when no broker reference is configured. Snapshots seed from `origin/<branch>` only and never inspect the host repo's currently checked-out branch. If the remote tracking branch or fallback cannot be prepared, planning fails instead of falling back to a stale local branch. Planning restart/continue actions reuse the preserved snapshot workspace so cancelled or interrupted provider sessions can still resume.
