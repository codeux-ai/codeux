...
In Docker execution mode, planning runs against a snapshot workspace and captures `.task-learnings.md` back out of that snapshot volume so memory capture still works even though the provider never writes directly into the host repo path. In `REMOTE` git mode, fresh planning invocations refresh `origin`, resolve the sprint feature branch when present or the repository's active branch otherwise, and use the effective runtime git default branch as the fallback. Snapshots seed from `origin/<branch>` only. If the remote tracking branch or fallback cannot be prepared, planning fails instead of falling back to a stale local branch. Planning restart/continue actions reuse the preserved snapshot workspace so cancelled or interrupted provider sessions can still resume.
