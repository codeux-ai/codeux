...
Hosted Jules CI repair uses the task session instead of a virtual-worker attention item, but follows the same durable-attempt rule. The gate writes a single-flight task-run checkpoint before sending the repair message, charges that attempt once after delivery, and holds the checkpoint until both a new PR head SHA is visible and the resumed session is terminal. Repeated polling of the same failed run, process restarts, an intermediate push, or a completed session without a push cannot send another message. If Jules asks for clarification during repair, `AWAITING_USER_FEEDBACK` remains part of the leased attempt and the normal clarification responder may answer without releasing it. Branch fallback CI is filtered to the current PR head SHA so stale failures never consume the next attempt.
...
Antigravity transcript parsing keeps permission grants, file reads, directory listings, command results, and other tool events as bounded tool telemetry. Only explicit response events can become assistant prose, and text-mode invocations select the final assistant response instead of concatenating intermediate narration. Worker/dashboard reply persistence applies the shared output sanitizer and message-size cap before a reply can be stored or forwarded to Jules.
