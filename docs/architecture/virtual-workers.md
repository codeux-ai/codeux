...
Before every provider invocation, Code UX reasserts ownership of that runtime volume for the container's effective non-root UID/GID. This boundary check covers newly created and previously root-owned volumes, preventing CI repair from failing before the provider starts because it cannot create provider configuration or cache directories.
...
Failed-job excerpts are error-focused rather than simple head/tail truncation. The collector scans the complete failed-job output for the named failed steps and assertion/error signals, then preserves bounded context around every relevant match (including expected/received output and source locations) while dropping runner provisioning and cleanup noise. Duplicate push and pull-request runs for the same head SHA and workflow collapse to one repair input. Final-merge CI repair receives this same structured evidence.
