...
- Mutation helpers that affect live runtime state invalidate the frontend live payload cache after successful writes. Project-scoped writes clear only that project's live payload entries, while mutation routes that do not carry project identity clear all live payload scopes so subsequent refreshes hydrate from `/api/live` without requiring a page reload.
