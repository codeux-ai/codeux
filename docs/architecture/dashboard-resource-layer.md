...
- Live payload cache invalidation is generation-aware. If an invalidated in-flight `/api/live` request later resolves, it may return to its original caller but cannot repopulate the cache or delete a newer in-flight request for the same project/sprint scope.
