...
The runtime workflow builds both targets, smoke-tests their tool inventory, emits SBOM and provenance attestations, and signs each published digest with Sigstore. GHCR mutation is restricted to the `release.published` event: dev pushes, schedules, and manual dispatches cannot publish. Runtime pull requests still build, smoke-test, and scan affected targets with only `contents: read`; the package-write and OIDC permissions exist only on the release publish job. Before publishing, the workflow verifies that the release tag matches `package.json` and that its commit is reachable from `main`. Channel tags are discovery pointers only. `ManagedRuntimeService` pulls the channel, resolves the local `RepoDigest`, verifies Node 24 in a network-isolated smoke container, and stores only immutable digests as the active runtime.
