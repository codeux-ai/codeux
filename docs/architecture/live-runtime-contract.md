...
4. **The Browser Renders (No Reconciling):**
   ...
   Optional status timestamps cross a tolerant display boundary before localization: ISO, Unix-second, Unix-millisecond, and legacy 12-hour time-only values are normalized to a valid `Date`, while malformed values suppress the **Updated** label. Animated shader backgrounds request a paint frame only after a bounded timer interval, so a no-vsync RAF implementation cannot busy-spin while this larger snapshot is rendered.
...
- **`execution`**:
  ...
  - **Sprint Workflow Evidence:** `sprintWorkflowProjections` remains in both full and lean execution snapshots. It carries the newest planning status per sprint plus only active, human/user-owned, unassigned task interventions. Unlike `attentionItems` and `recentInvocations`, this compact workflow evidence is not truncated by activity-feed limits.
