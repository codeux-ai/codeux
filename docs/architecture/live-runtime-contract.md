...
- **`execution`**:
  ...
  - **Selected Sprint Context:** `/api/live` passes the selected sprint id into execution snapshot assembly. The execution projection protects the selected sprint from global recency caps by retaining its latest sprint run, current task dispatches, recent runtime events, and invocation records even when other active or recent sprints dominate the capped project-wide feeds. Selected-sprint queries remain scoped to the requested project, so a stale selected sprint id from another project does not leak execution data across project boundaries.
  - **Live Invocation Feed:** The execution projection includes `recentInvocations` from `execution_invocations`, merging the latest project-wide records, all records for expanded active/paused/queued sprint runs, and all records for the selected sprint. The Live page renders selected-sprint invocations beside the runtime feed so operators can inspect paused or stopped sprint history even when other sprints are active. Task cards also derive task-scoped invocation feeds from this same list using task, dispatch, and task-run identity. Full transcripts remain linked through the Chat invocation view.
