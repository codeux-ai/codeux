...
The viewer uses a sandboxed iframe `srcdoc` document so generated dashboard code never runs inside the main Preact bundle. For validated TSX/Preact revisions, it prefers the persisted Vite `dist` viewer artifact from revision runtime metadata and inlines the artifact's HTML, CSS, and JavaScript into the frame document. Validated direct HTML or browser-ready JavaScript revisions use the immutable source entry path instead: validation still materializes and builds the server-controlled workspace in Docker, while publication injects only the frozen browser bridge into the selected declared legacy route entry and never imports host application modules. The frame receives a frozen `codeUxDataBridge` / `CodeUXCustomDashboard` object and can request only declared sources or external routes. Published requests cross a frame-source, opaque-origin, per-frame-session `postMessage` boundary; validation harness requests carry their owning validation session. Both modes reach `CustomDashboardRuntimeService`, which verifies project/dashboard/revision ownership and active publication or session state. The runtime selects a sole required credential slot automatically, requires an explicit declared slot when required selection is ambiguous, and preserves unauthenticated access when no required slot is unresolved. It rejects invalid selection and binding metadata before egress. External credentials resolve inside `CredentialBroker.withResolvedCredentialId` and pass only as trusted headers to `EgressPolicyService`, which enforces explicit hosts, ports, methods, content types, timeouts, redirects, rate limits, and bounded bodies. Browser authorization, cookies, dashboard session headers, sensitive upstream headers, and upstream error bodies are not forwarded.
