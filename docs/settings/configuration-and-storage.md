...
- provider session DB at `~/.code-ux/session-tracking.db`; it stores provider lifecycle, branch, and activity projections. Jules prompts remain there for hosted usage estimation, while local CLI prompts are not duplicated because their durable invocation messages already live in `app.db`. The session schema upgrade clears pre-existing local prompt copies once and preserves Jules prompts.
