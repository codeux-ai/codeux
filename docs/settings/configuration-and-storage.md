...
Design guidance is project/sprint-scoped under `designGuidance`. It stores selected tech-stack guidance, selected styleguide guidance, `hideDefaultStyleguides`, and custom tech stack/styleguide entries with stable `id`, `name`, `summary`, and `instructionMarkdown` fields. System defaults resolve both selections to `none`, so existing and imported projects receive no design styleguide by inheritance. New local and new remote project initialization writes an explicit project override selecting the generic Code UX award-winning styleguide; imported local or Git projects remain at `none` until an operator or setup flow changes them. Planning and Project Setup prompts receive a compact `Project Guidance` section only for selected non-`none` entries, so generated tasks and setup artifacts can reflect active guidance without duplicating inactive defaults.
