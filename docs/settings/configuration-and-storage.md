...
- Agents settings translate reflection controls and validation only. Criterion ids, labels, prompts, thresholds, agent instructions, and memory templates remain stored exactly as authored.
- Persistent-skill storage records and agent-storage attachments save immediately rather than joining the Settings draft. The localized manager preserves mutation suppression, typed deletion confirmation, focus restoration, and failed-operation recovery.
