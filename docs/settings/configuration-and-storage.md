...
- Additional Git branching and PR-title behaviors configured here include `git.featureBranchPrefix` (e.g. `feature/codeux/`), `git.sprintBranchScheme` (e.g. `feature/sprint{sprint_id}-implementation`), `git.sprintKeyPrefix` (uppercase identifier such as `SPR`), and `git.taskPrTitleScheme` (default `({sprint_tag}) {task_title}`). Task PR title templates support `{sprint_tag}`, `{sprint_key}`, `{sprint_number}`, `{sprint_title}`, `{task_key}`, `{task_title}`, and `{provider}`; `{sprint_tag}` uses the first linked issue key when present, otherwise the sprint key prefix plus sprint number, then a stable sprint slug/id fallback.
