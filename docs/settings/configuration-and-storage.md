...
- local cluster settings sync uses an MCP Streamable HTTP client with `Authorization: Bearer [REDACTED]` and calls the remote `manage_settings` tool; it does not introduce a separate unauthenticated settings API. Mutating sync requests use the same two-step `manage_settings` approval flow as any other settings mutation: the first call records an approval requirement, and a later caller-confirmed retry sends the exact same patch payload with `approval.confirmed: true`.
- cluster sync only builds payloads enabled by the saved connection policy. `systemSettings` sync patches `runtime`, `defaults`, `mcpTools`, `customMcpServers`, and `modelPricing` while blanking inherited Git/Jira tokens from project defaults. `providerSettings` sync patches `integrations.providers`; local auth paths, mounted auth flags, login timestamps, and token file contents are not uploaded unless a future artifact-aware flow explicitly implements that policy. Errors and structured logs redact bearer tokens, provider API keys, and other auth material before they are returned or emitted.
