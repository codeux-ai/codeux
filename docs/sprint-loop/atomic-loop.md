...
- Sprint planning follows the same restart contract. Startup preserves the full planning request and options, then continues the exact native provider conversation in its stable planning workspace. It never replaces a recorded but unavailable planning conversation with a fresh one; only a request stopped before provider linkage is reissued from durable input because no conversation existed yet.
