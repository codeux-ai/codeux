...
3. **Sync sessions**: Synchronizes hosted provider sessions and local/CLI/worker dispatch state through execution records and provider invocations. Sync source is provider-agnostic. Jules state synchronization uses one shared, short-lived, newest-first session page across every sprint and admission caller; an exact-session read is used only for a durable task outside that page. Activity history is telemetry and is never awaited by this scheduler step.
...
5. **Start ready tasks** (`orchestrate` only):
   ...
   - The scheduler asks the admission service for current purpose-aware capacity once per provider in the cycle, caches that budget, and decrements it after every successful start. Adaptive reservations therefore stop over-dispatch before lower-level claims become visible; configured capacity remains an upper bound rather than a promise that every slot is presently available.
   ...
   - Pending worker clarification is also a retryable dispatch deferral, not a failed coding attempt. The planning row stays `in_progress`; every ordinary start is refused at the dispatch boundary until the exact clarification continuation creates a new dispatch. Paused or terminal sprint runs cannot generate or deliver an automatic manager answer.
...
- The watch loop holds one project Git-helper lease for its lifetime. The helper is created lazily on the first eligible Git command, shared by concurrent sprints in the same project, and removed after the final active sprint releases its lease. A single-cycle orchestration action uses the same scoped lease.
...
- Provider completion is only an intermediate CLI-workflow checkpoint. Task QA and branch merging wait for durable Git-finalization evidence (`cli_git_pushed` or `cli_git_no_changes`), so they never inspect or settle a branch that is still being materialized.
- If shutdown lands after a coding provider has completed but before Git finalization records the task as code-complete, startup identifies the exact run through the recovered CLI session even when a hard kill prevented the shutdown event from being written. Recovery preserves the workspace, requeues the prematurely projected terminal run, and marks the completed provider result. The replacement task run resumes at Git finalization instead of invoking the coding agent a second time. A missing preserved workspace falls back to a normal fresh invocation rather than trusting unavailable changes.
- Under the restart invocation `continue` policy, startup treats QA review, CI fix, and merge-conflict repair as durable work streams just like task coding. It closes each interrupted audit invocation, preserves its logical/native session and workspace binding, and requeues only the work needed to create the correlated continuation. Claimed repair attention is released from the stopped virtual-worker endpoint before the next worker poll. A task-coding session interrupted by the runtime restart receives an idempotent guardrail refund keyed to its task run, so repeated operational restarts cannot exhaust the coding-attempt budget; genuine provider failures remain charged.
- Sprint planning follows the same restart contract. Startup preserves the full planning request and options, then continues the exact native provider conversation in its stable planning workspace. When Codex native telemetry contains a session id, recovery addresses that exact id. When an older Codex invocation has no native id but its workspace was preserved, recovery uses workspace-local `exec resume --last`; the logical Code UX planning/session/workspace id is never passed as a Codex native resume id. It never replaces a recorded but unavailable planning conversation with a fresh one; only a request stopped before provider linkage is reissued from durable input because no conversation existed yet.
...
- Antigravity transcript ingestion treats permission reports, file views, directory listings, command results, and other non-response model events as tool telemetry rather than assistant prose. Text-mode consumers use only the final assistant response, and persisted worker/dashboard reply bodies are sanitized and capped at the normal invocation-message limit. This prevents large tool envelopes from being prepended to replies, forwarded to hosted sessions, or retained as unbounded assistant messages.
...
- A coding agent's reasoning-based blocked marker is not sufficient for an attention-free terminal block. A no-change blocked turn creates or reuses a durable worker clarification, ends the provider invocation as a completed turn, and schedules the configured Project manager. The task resumes in the preserved workspace and native CLI conversation after the answer. If manager generation, guardrails, Git credentials, runtime configuration, or continuation lineage prevents recovery, an active human attention item remains visible.
- A confirmed no-change completion remains separate from blocking: the task advances to `CODING_COMPLETED`, allowing the normal task-completion/no-change QA trigger to verify it when that QA policy is enabled.
- If the current hosted Jules session becomes active again after a terminal observation, session synchronization reopens its existing provider-usage and linked execution invocation lifecycle without refetching the full transcript. When the remote session is terminal, the same synchronization settles provider-usage and linked execution-invocation rows even if the task run and dispatch were already terminal before that cycle. A session attached to an older task run is never reactivated after a newer retry exists: startup recovery closes only the superseded local audit rows while leaving the remote session untouched. The task, dispatch, provider-capacity ledger, and Live invocation therefore agree during post-completion QA, CI, or merge continuation work without creating duplicate running projections.
- Jules action-required episodes have a durable epoch in task-run events. Successful plan approvals, clarification replies, and resume nudges remain reply-pending across watch cycles and process restarts, preventing duplicate mutations while the provider state is stale. The epoch includes the provider's remote update revision: if Code UX is offline while the session resumes and later returns to the same waiting state, the newer revision starts a fresh episode even though the state name did not change locally. A clarification that remains waiting past its configured cooldown is escalated to a human instead of oscillating indefinitely between `RUNNING` and `BLOCKED`.
- Jules feature-PR CI repair is single-flight. Before sending a repair message, the gate durably claims the exact failed observation and records the guardrail attempt idempotently only after delivery. The claim survives restarts and stays active until the PR head commit changes **and** the resumed Jules session finishes, so repeated watch cycles cannot send the same failure again or exhaust the budget. Action-required states such as `AWAITING_USER_FEEDBACK` remain inside that attempt: normal clarification automation may answer Jules, but the CI gate cannot send another repair prompt. CI runs used as fallback checks are filtered by both branch and current PR head SHA; after a push, an older failed run cannot trigger another repair while the new commit is waiting for CI.
- Watch cycles emit a deduplicated `watch_cycle_stalled` event when a cycle exceeds the larger of 30 seconds or three configured polling intervals. Completion after that point is logged as recovery, and watchdog telemetry failures cannot fail the cycle.
- The local session snapshot used by the watch loop projects only routing and state metadata; it does not load stored CLI prompts. Full prompts remain available through direct session and invocation reads, but large QA/planning prompts are not decoded again on every one-second synchronization cycle.
- Feature-PR gate evaluation loads the latest task runs for the DAG in one chunked batch and uses one immutable, event-type-filtered local-Git snapshot per task per cycle. It reuses those run objects through branch recovery, merge settlement, and CI event writes instead of issuing per-task lookup queries. Duplicate idempotent task-run events do not invalidate runtime wall-time caches or publish realtime refreshes.
- LOCAL Git-finalization evidence uses the same chunked latest-run projection for the whole DAG. Repeated status-derivation and merge-protocol passes therefore avoid one latest-run query per task, including pending tasks that have no run yet.
...
- Dashboard live snapshots are optimized for high sprint concurrency: selected-sprint checks use targeted project/sprint lookups instead of hydrating every sprint, and recent Jules activity is fetched incrementally at no more than one bounded page per refresh. Per-session requests are coalesced and cached, use abortable deadlines, retain at most 50 compact activities, discard patches/media/shell artifacts, and reject oversized live responses before caching. Provider activity event reads use partial sqlite indexes for activity-only paths.
...
- A provider invocation is capacity-consuming only while it is `running` with no `finishedAt`. Jules invocations linked to action-required `BLOCKED` task runs stay durable but release execution capacity until the provider resumes. Repository writes enforce the inverse lifecycle invariant as well: returning an invocation to `running` clears terminal timestamps, while every terminal status receives a finish timestamp.
- Task cards retain successful QA evidence after completion, but they do not project an older running, failed, cancelled, or changes-requested review as current state after the task is merged or otherwise completed. The historical QA run remains available in execution history.
- Loop exits when:
...
- When QA requests fixes and Code UX applies them through a same-session CLI follow-up, the next sprint cycle treats the completed `cli_task_followup` invocation as fresh task work and reruns QA verification instead of waiting for a separate task-run completion timestamp. The task remains `CODING_COMPLETED`/`QA_PENDING` between handoff settlement and verification dispatch, closing the restart window in which ordinary coding could otherwise be launched. If a restart happens before that invocation marker is persisted, the cycle can also use a later completed task run for the task's current session as recovery evidence, preventing a completed fix attempt from staying parked forever. Codex continuation pairs the workspace-binding task run with its coding invocation and reads only that native thread's rollout from the paired runtime home. The one-time fresh-session fallback remains for legacy or corrupt records whose rollout is already unavailable; strict exact-session recovery remains fail-closed.
- QA reviewer execution is correlated by exact invocation id and reviewer preset, with a distinct logical session and isolated workspace for each reviewer row. The execution audit row is created before snapshot preparation for complete observability; if workspace preparation fails before a provider claim is linked, that row is failed immediately with the preparation error instead of remaining falsely `running`. Restart continuation reuses a successfully prepared workspace and resumes the provider-native conversation only while completing the same interrupted reviewer cycle. Once a decisive verdict is saved and follow-up work can change the branch, the next verification cycle creates a fresh snapshot instead of inheriting the pre-fix review workspace. A `changes_requested` row records `pending` before its coding continuation begins, so the next cycle can resume an interrupted handoff or settle an already-completed follow-up instead of heartbeating forever behind the saved verdict.
- Hosted QA coding follow-ups are asynchronous. API acceptance persists `awaiting_provider`, keeps the task `RUNNING` with `QA_PENDING`, and remains a pending continuation for budget calculations. Restart reconciliation never resends that follow-up while its task run is active. Only a matching task run that completed after dispatch settles the checkpoint and admits the final verification review; an accepted but unfinished hosted request cannot prematurely exhaust QA or hand the task to a human.
...
- Wide-DAG QA reconciliation and initial merge-gate evaluation load latest-cycle rows and decisive/terminal cycle counts for all tasks in one chunked query. Per-task review dispatch and post-review gates retain their exact fail-closed behavior without four SQLite reads per task on every watch cycle.
- Task QA uses bounded waves of at most four reviews per orchestration cycle, or a lower positive routed-provider capacity. After one wave is scheduled, the cycle settles its results, merges ready branches, and starts newly unblocked coding before admitting another QA wave. The provider admission service remains authoritative and can lower concurrency further under host pressure before an invocation claims a slot.
...
- CI autofix retries are capped by `julesCiAutofixMaxRetries` (the legacy-named setting); once exhausted, the task is escalated as intervention-needed with exact task id, PR URL, failed check names, the newest failed-run summary, and every failed job name (focus: fix CI before merge). The cap applies to the generic CI-fix loop, including worker repairs. One worker attempt is charged idempotently immediately before an actual provider invocation. Workspace setup, restart continuation, and host publication reconciliation cannot consume extra attempts. A no-change provider result resets publication state so a retry invokes the provider again instead of rapidly replaying the same empty publication.
...
- Provider-backed clarification replies run outside the watch-cycle critical path. The cycle durably records the worker start, keeps the task in progress, and continues session sync, DAG admission, QA, and CI while the reply worker runs. An in-process reservation prevents duplicate workers for the same action-required epoch; completion or failure is persisted, and the provider run is aborted at the configured worker timeout so a wedged reply cannot retain a slot indefinitely.
...
- Worker and feature-gate CI guardrail handoffs share one stable deduplication key and persist the exact ledger subject. Resolving either a current or legacy handoff resets that subject's `ci_fix` counter, including taskless final-merge subjects, so the operator receives one fresh bounded repair cycle rather than an immediate re-lock.
