...
- Task-completion and completed-without-PR QA use the trigger's ordered `agentPresetIds` list. Each configured reviewer creates its own `qa_review_runs` row in the same review cycle with the same `run_index`; an empty list resolves exactly one default QA reviewer. The cycle passes only when every reviewer passes. Any reviewer that requests changes, fails, or is still running keeps the task blocked under the existing QA gate rules.
...
- QA review budgets count review cycles, not reviewer rows. Multiple reviewer rows with the same `run_index` spend one task or sprint QA attempt while still preserving reviewer-specific `agent_preset_id`, `agent_name`, payload details, and task-run events for dashboard history.
