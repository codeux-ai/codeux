...
| Delivery workflow badge | `controlFeedback`, `enterExit`, `expansionCollapse`, `asyncFeedback` | One bright interactive badge exposes Coding → Pull request → QA → CI → Merge → Completion on Live, Tasks, Sprints, and Overview. The interaction boundary has no visible outer card: the opaque Delivery flow card, floating responsive arrow, and optional opaque QA card remain independent siblings. Reduced motion keeps connectors and the arrow static. QA edits retain the blue pencil treatment; red identifies provider/runtime/workflow failure or explicit active human-only task attention as **Human needed**. Resolved, worker-actionable, and ambiguous sprint-run summaries fall back to the ordinary stage. |
