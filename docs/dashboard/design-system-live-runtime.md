...
- The initial Live snapshot rehydrates persisted evidence after server restart or browser reconnect while the existing snapshot remains visible during transport recovery. Newer settled events replace older failed/waiting events, resolved or dismissed attention stops forcing failure, and settled merge metadata prevents stale event resurrection.
