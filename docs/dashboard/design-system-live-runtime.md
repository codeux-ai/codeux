...
## QA Review And CI Workflow Projection

- Live task cards show the shared QA and CI badges beside lifecycle and merge metadata. A persisted QA summary remains visible during active phases and reconnect recovery: green check means pass, signal progress means running, blue pencil/**QA edits** means completed changes requested, and red X/**QA failed** means the provider run failed, errored, or was cancelled without a usable verdict.
- The QA badge opens a viewport-positioned named region on hover, focus, or activation. It exposes available outcome, summary, findings, fix instructions, target task key, reviewer/time, and follow-up task data. Generated **Follow-up task N** specifications begin collapsed; their `aria-expanded`/`aria-controls` buttons reveal title, description, priority, dependencies, and the full prompt. Outside pointer/touch or pointer departure without contained focus dismisses the card; `Escape` restores trigger focus. Responsive clamping, bounded vertical overflow, keyboard/touch activation, and reduced-motion static cues match Sprints and Tasks.
- Live CI always projects Pull request, Checks, and Merge with the first-class states `pending`, `in_progress`, `successful`, and `failed`. The UI presents `in_progress` as running and `successful` with a green check. Only failed checks, merge conflicts, and failed merge attempts receive a red X. Review blockers remain pending after successful checks, merge conflicts fail Merge, and QA provider failure is not folded into CI.
- `live-session-view-model.ts` indexes scoped tasks, dispatches, runtime events, invocations, and attention before card construction. CI is narrowed to the selected sprint and the latest dispatch's sprint run, then the newest matching persisted `ci_gate_status` event is selected by creation time and event ID and combined with active open/claimed CI-repair attention. Durable task merge metadata is fallback evidence; cards do not start their own polling loops.
- The initial Live snapshot rehydrates persisted evidence after server restart or browser reconnect while the existing snapshot remains visible during transport recovery. Newer recognized settled gate events replace older failed/waiting events for the same task and sprint run, and resolved or dismissed attention stops forcing failure.
