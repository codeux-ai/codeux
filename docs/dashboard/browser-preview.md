...
- The Browser page right sidebar keeps Launch Container expanded at all times. Selected Sprint, Environment, Runtime notes, and Container logs start collapsed; the Selected Sprint header keeps the current port mapping visible as a badge, or `port pending` when no running preview is selected.
