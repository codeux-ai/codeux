...
- Attached storage chips disclose bounded storage descriptions, skill counts, names, tags, and short content previews only after their first pointer hover or keyboard focus. Loading, empty, retryable error, preview truncation, and server-truncation states must remain explicit; detached storages never trigger a contents request.
