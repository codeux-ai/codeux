...
The form validates absolute dates, target-specific values, positive recurrence intervals and counts, and recurrence end windows before converting values to ISO. Each validation message is associated with its control through `aria-invalid` and `aria-describedby`; an invalid submit preserves the draft and focuses the first invalid control. Duplicate submissions are suppressed while a save is in flight, and overlapping refreshes ignore stale responses.

Pause/resume and delete mutations are tracked independently by entry ID and action. Only the action currently running is disabled, repeated activation of that action is suppressed, and other rows remain operable. Successful pause/resume work is announced in its row. A failed mutation leaves a persistent row alert with a Retry action, while a successful delete also produces explicit status text so animation is never the only success signal.

Deletion uses the shared `ConfirmDialog`, not native browser confirmation. The localized dialog names the schedule, keeps its confirmation action pending while deletion runs, supports Cancel and Escape, and restores focus to the originating Delete action when dismissed. After a successful deletion removes that action, focus moves to the first action in the next surviving row, the previous row when there is no next row, or the Scheduled entries heading when the list becomes empty.

Refreshes retain the last usable entries and occurrences while new data is loading and after refresh failures. The existing refresh sequence guard remains authoritative: an older response cannot replace newer scheduler data. A retry starts a new refresh without clearing the cached list.

Tabs, target filtering, conditional form sections, row/list changes, and async feedback use the dashboard's semantic interaction contracts (`selectionMovement`, `expansionCollapse`, `listReorder`, and `asyncFeedback`) instead of page-local durations. With reduced motion enabled, those contracts resolve to zero-duration state changes while selected, pending, success, and error text remains available. Controls that survive a filter or view change retain focus; when a list mutation removes the focused row action, focus follows the surviving-row behavior described above.
