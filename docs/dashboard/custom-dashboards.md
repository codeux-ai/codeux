...
The declaration fields `headerName` and `scheme` describe how the broker should attach a credential and may safely contain values such as `x-api-key`, `authorization`, and `Bearer`. Static header values and credential-shaped keys such as `apiKey`, `api-token`, `access-token`, `secret`, and `password` are not configuration and are rejected.
