...
- Expanding the sidebar opens directly to the current alive memory list for the selected tier, sprint, and agent filter set, with a visible search field for filtering by memory text or category. Browsing all visible memories is still the default path; search is not required before the list is useful.
...
- Added keyboard-accessible clear search functionality to `MemorySearch.tsx` (supports clearing via `Escape` and a dedicated clear button with an explicit `<kbd>Esc</kbd>` visual affordance). Typing in the search field debounces sidebar filtering so dense memory lists remain responsive while the user enters a query.
