...
- On `/tasks` and `/live`, the selectors atomically replace the router's workspace scope. Project changes write the new `projectId` and discard a sprint from the previous project; sprint changes write the active `projectId` and new `sprintId`. A destination consumes each new route `projectId` once to initialize the shared project selection, then follows later shared selection changes without re-applying that query value. This keeps deep links shareable while ensuring an older mounted tab cannot reclaim selection or make multiple tabs oscillate between projects.
