...
## Focus, Cancellation, and Pending Work

The setup shell uses the shared dashboard `Dialog` behavior. When the setup form becomes available, focus starts on the active step heading. Tab and Shift+Tab remain inside the dialog, Escape follows the same close policy as the close button and backdrop, and closing restores focus to the control that launched onboarding when that control is still available. Each step change focuses the new heading during layout, without a timer. A failed progression focuses the first invalid field (or the validation summary when no field target exists), while moving backward or rechecking readiness retains every value in the in-memory draft.

The reducer records a fingerprint of the initially loaded settings, setup mode, and provider selection. Closing a changed draft opens a nested confirmation with separate **Keep editing** and **Discard and close** actions. Escape or the confirmation backdrop only returns to the draft; neither can silently discard it. A clean draft can close directly.

Settings save and dependency installation are non-cancellable phases. While either is active, close, Escape, backdrop dismissal, step shortcuts, Back, and Next are unavailable, and visible/live status text explains which operation must finish. Readiness checks remain safely cancellable: closing invalidates the request generation, stops runtime-asset polling, clears the appearance preview, and ignores late readiness, provider-preparation, installation, or save results. Reopening onboarding starts a fresh draft and request generation.

Step announcements combine the localized step number and label with the current readiness, installation result, installation error, save state, or draft state. Installation success, partial completion, and failure remain visible text in addition to any motion. Reduced-motion mode resolves `enterExit`, `selectionMovement`, and `asyncFeedback` durations to immediate state changes while preserving headings, progress values, static highlights, and live status copy.
...
An installer error retains the attempted mode so the error panel can retry that exact installer even when no structured result was returned. Partial and failed structured results also retain their mode for retry. Every completed retry refreshes readiness without replacing setup values already entered on other steps.
...
The tour card includes previous/next controls, a skip action, the current step count, status copy, and a progress bar. Its shared focus trap contains Tab and Shift+Tab, Escape closes the tour, ArrowLeft/ArrowRight move between steps, and the primary Next/Done control receives focus as the step changes. Previous, Next, Skip, and Finish controls include the target step or current location in their accessible names. Closing the tour through Escape, Skip, or Done restores focus to the control that started the tour when possible. Delayed starts, geometry polling, progress intervals, keyboard listeners, and GSAP work are cleared on close or unmount so a stale tour cannot reopen or update after its owning surface is gone.
