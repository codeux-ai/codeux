...
Internal MCP clients use the same importer services through `manage_sprints` action `import_issues`. For Jira, search-based imports and explicit-key imports both attach the selected issue descriptions to planner scope, and `includeConversation` controls whether comments are included with the imported description. For payload examples covering search-only imports, assigned-work searches, explicit Jira keys, explicit GitHub/GitLab issue numbers, explicit external IDs, canvas identifiers, sprint attachment, and plan-after-import flows, see [MCP Tools and Contracts: `manage_sprints import_issues`](../mcp/tools-and-contracts.md#manage_sprints-import_issues).
...
When the sprint is submitted, selected issues are persisted as linked sprint issue records and the sprint prompt receives a structured `Linked Issues` markdown section. Each imported issue is appended with source metadata, labels, assignees, author and timestamps when available, the complete issue body, and the selected conversation context. The stored linked issue record keeps the imported body, conversation selection, author, timestamps, and non-secret metadata across sprint reloads and updates. This gives the Planning agent and task agents the actual issue text instead of only a remote link. If a later sprint update carries linked-issue metadata without body or conversation text, Code UX preserves the existing linked-issue prompt text instead of replacing it with an empty-body fallback.
...
Selected Jira issues are loaded through the same prompt-context path as GitHub/GitLab imports. The sprint prompt receives the full Jira description and, when `Append Conversation` is enabled, Jira comments. Jira search results keep a concise description preview for cards while carrying the complete description markdown through backend search results so planner prompts do not depend on the truncated preview. Imported Jira cards are persisted as linked sprint issues with provider `jira`, host extracted from the Jira URL, project key, repository fallback, parsed issue number from keys such as `OPS-42`, issue key, labels, assignees, status, source URL, the selected conversation flag, and the imported prompt context. Stored linked issue context survives sprint reloads, prompt edits, and replanning so the Planning agent continues to see the imported Jira description from the linked sprint issue record instead of relying on the original search response. The import result cards also surface Jira issue type, priority, reporter, assignee, labels, status, updated timestamps, and a description preview when Jira returns those fields.
