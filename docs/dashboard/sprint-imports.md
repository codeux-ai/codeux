...
The modal also includes quick presets for common backlog triage paths: open backlog, recently updated work, assigned-to-me or text-user matches, security-labeled items, quality and tech-debt items, failed-CI follow-ups, and merge-conflict follow-ups. The results pane supports bulk selection, bulk conversation toggles, linked-issue imports, and special-task imports for security, quality, failed-CI, and merge-conflict selections so operators can move remediation work into the sprint without hand-writing task prompts.
