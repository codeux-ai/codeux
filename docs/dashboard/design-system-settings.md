...
*   Category rail buttons expose selected, pending, and disabled state through visible labels plus ARIA (`aria-current`, `aria-busy`, `aria-disabled`). Category movement uses `selectionMovement`; reduced-motion users receive the same static selected and pending labels without relying on animated movement.
