...
4.  **Metadata and Hierarchy**:
    ...
    *   Every visible `SectionCard` subcategory exposes card-level help controls in the header action area: an info icon that opens keyboard-accessible guidance and a docs icon that links to the matching `/docs/settings-subcategories-<subcategory>` subpage. The legacy `/docs/user/dashboard/settings#<subcategory>` path remains a compatibility route. These controls supplement row-level info affordances and must not replace field-specific help.
    *   Subcategory help metadata lives in `dashboard/src/v2/lib/settings-subcategory-docs.ts`; canonical long-form pages live under `docs/settings/subcategories/`, and the published mirror lives in `docs-web/settings/subcategories/`. Add or update all affected surfaces when creating, renaming, or removing a settings subcategory.
...
*   Smart Find keeps focus in the search field while typing and when the clear control is used. Result status remains polite; idle status avoids visible category-count copy, while active searches include count/category context plus match previews. Empty states include recovery terms instead of leaving the rail blank without explanation.
