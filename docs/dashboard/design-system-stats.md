...
- Do not render a second Trend KPI band inside the studio. The mode metric deck owns total tokens, invocations, active time, cost, and cache-rate summaries; the Trend studio starts with the interactive graph and keeps compact secondary duration, velocity, and completion signals below it.
...
- The compact chart header keeps filter access and zoom reset visible near the graph title. A single wrap-safe toolbar line summarizes selected range, bucket count, resolution, and active zoom without creating another nested panel.
...
- Hover, keyboard focus, minimap selection, drag zoom, and active bucket controls all update the same bucket-inspection summary; avoid a second live-values panel that repeats those values. Before explicit focus, the summary exposes the latest visible bucket so exact values are never hover-only. The inspection surface fills the height of its chart-side column, caps to the graph height, and scrolls internally when the graph is tall.
- The bucket inspector distinguishes latest, focused, and pinned inspection states in visible copy and polite tooltip text. Marker position has a text equivalent such as `Focused bucket marker at 45 percent of the visible chart window` so color and motion are not the only cues.
...
- Panels and stat cards are grounded warm void work surfaces with solid `--stats-surface-*` tokens and hairline borders. Shared structural surfaces do not use gradients, glass blur, elevation shadows, or hover lift; telemetry accents stay confined to semantic borders, status fills, and data visuals.
...
- Metric cards with sparkline micrographs use the standard card surface, not a separate muted graph background. Their labels, values, supporting copy, quality hints, and sparkline slots use wrapping-safe, width-bounded layout so long provider or model content cannot widen the page. Populated sparklines expose a concise `role="img"` summary with point count and high/low values; empty sparkline slots show a static `No sparkline data` label and an explicit no-data `role="img"` description.
...
- Invocation tables provide captions, explicit `aria-sort` state on every sortable column, explicit sort button labels, mobile cell labels, and wrapping-safe cells for long provider, model, error, and transcript text.
...
- System invocation tables preserve semantic header relationships: desktop headers use `scope="col"`, sortable headers set `aria-sort` to `ascending`, `descending`, or `none`, and body cells reference their headers.
