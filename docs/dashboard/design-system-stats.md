...
- Filter menus focus the first useful series control when opened, close on Escape, and restore focus to the trigger when possible. Reset actions announce the resulting active series count. The last enabled series remains focusable with `aria-disabled` and an explicit reason so operators can discover why it cannot be turned off.
