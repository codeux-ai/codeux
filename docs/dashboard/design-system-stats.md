...
- Chart announcements should describe completed operator actions, not pointer movement frames: focused bucket changes, pinned buckets, drag/minimap zoom ranges, zoom reset, series enable/disable counts, last-series guard reasons, refresh/stale/error status, and empty data recovery. Dragging may update selection bounds visually, but live-region copy should fire only when a zoom is committed or reset.
