...
*   **QA Review Verdicts:** The shared sprint/task review badge normalizes status and outcome values across case plus underscore, hyphen, and whitespace separators. Running reviews retain the progress treatment, successful reviews retain the green check treatment, and requested changes use a blue edit treatment with visible `QA` text in regular and compact layouts. Hover and keyboard focus open the same viewport-clamped, mobile-safe detail overlay; requested-change overlays identify the verdict explicitly and retain every available summary, finding, reviewer, and review-finished value. Reduced motion removes decorative pulse, spin, and transition behavior without removing state copy.
