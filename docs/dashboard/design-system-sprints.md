...
*   **Sprint Attention Indicator:** `SprintAttentionIndicator` gives gallery cells and ledger rows one attention-state contract derived from the effective sprint status presentation. Failed sprint execution takes precedence and uses a red-bordered outer treatment plus a pulsing exclamation indicator. In the gallery, an eligible human-owned intervention uses an amber person/`zZZ` waiting indicator exactly `10px` above the cell without adding an alarm border; worker- and system-owned attention, including worker-actionable merge conflicts, must not be presented as waiting for the user. Ledger rows retain their compact inline attention treatment.
...
*   **Stable Sprint Workflow:** The unified workflow badge replaces separate QA and CI disclosures while the sprint lifecycle remains independently readable. During `running`, the sprint workflow stays on Coding and suppresses aggregated child-task PR, CI, and Merge transitions; individual task surfaces own those changing gates. A completed sprint settles the full workflow rail successfully even when historical gate evidence is absent from the latest snapshot.
...
*   **Durable Projection:** Sprint CI derives from the newest persisted `ci_gate_status` event per task and `main_merge_gate_status` event for the final merge, ordered by creation time and event ID, plus active open/claimed CI-repair attention. Persisted merge metadata is fallback evidence only. Project snapshots rehydrate this state after restart/reconnect; a newer recognized settled gate event replaces older failure evidence for the same entity, and resolved or dismissed attention no longer forces failure.
