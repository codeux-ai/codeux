...
*   **Sprint Attention Indicator:** Gallery cells and ledger rows share one attention-state contract derived from the effective sprint status presentation. Failed sprint execution takes precedence and uses a red execution-failure indicator; eligible human-owned intervention uses a waiting-person indicator with visible `zZZ` copy. Both states apply a red outer treatment to the complete cell or row, retain the normal status, progress, review, links, and actions, and expose explicit status text to assistive technology. Worker- and system-owned transient pauses must not be presented as human intervention.
*   **Motion And Density:** Attention animation uses the shared motion timing and `motion-safe`/`motion-reduce` contract. Reduced-motion mode keeps the indicator and text static. The ledger variant remains compact, and desktop attention borders continue across every table cell while mobile preserves the complete row border and `mobileLabel` semantics.
