...
- Top-nav techstack selector sits beside the project and sprint selectors. It reads the active project's effective `techstack` selection plus the system-owned techstack catalog, displays `None` when `selectedTechstackId` is `null`, and keeps imported/unclassified projects explicitly unassigned until an operator chooses a catalog entry. Saving from the navbar writes only the project `techstack.selectedTechstackId` and `techstack.applicationKind` fields, clears cached effective settings for that project, and never mutates the system catalog. New-project creation remains the only dashboard flow that may prefill the catalog default as an explicit project override.
