...
- The flat command band owns page orientation once: title, selected project, generated snapshot time, sprint lens, time-window controls, and the six-mode selector. The single page-level metric deck follows it, then the active workspace begins with a compact orientation row.
...
- The workspace stays one level deep: solid panels, hairline dividers, compact radii, neutral controls, and wrapping-safe rows. Avoid duplicate KPI bands, visible chart-summary card decks, nested decorative cards, repeated context strips, glass blur, gradients, shadows, hover lift, and metadata chips that restate the selected window or mode.
- Stats component styling is owned by shared flat Stats primitives and semantic tokens (`PANEL_CLASS`, `SUBPANEL_CLASS`, `CHIP_CLASS`, `INPUT_CLASS`, `STATUS_TONE_CLASS`, tab classes, empty states, ledger row classes, and track classes). Light and dark themes resolve structure through surface tokens; data colors are reserved for chart series, token/churn anatomy, source confidence, and operational status.
- Trend mode is the chart-first workspace. It includes a compact graph toolbar, responsive plot, minimap, latest/focused/pinned bucket inspection, grouped series switches, screen-reader data table, secondary signals, and purpose activity. Filter switches change visible series only; drag and minimap zoom change only the visible buckets and do not change the selected time window.
...
- System mode is an administrative invocation workbench organized around Sprint State, Health Snapshot, External API Activity, Error Categories, and Invocation Records. Operators choose `All`, `Errors`, or `System Msgs`, apply server-projected filters and sorting, page through records, expand a row, and inspect or retry that row's transcript in place.
- System filtering covers search, status, purpose, provider, error category, record view, sort, page controls, clear-all, active-filter counts, and result counts. These controls wrap rather than clipping on narrow screens. The body is project-scoped and queries invocation records independently of the snapshot window; do not describe the top time selector as an invocation-record filter.
- Phone controls wrap before layouts stack, tablet and desktop layouts introduce columns only when space permits, and dense rows use mobile labels or internally bounded overflow instead of page-level horizontal scrolling. Reduced motion snaps shell, selection, list-reveal, reorder, validation, and refresh transitions while preserving visible state text and focus.
- Loading, no-data, low-data, empty, and error states reuse the Stats shell. Cached snapshot values remain visible during refresh. Loading and reduced-data panels use `role="status"`; blocking errors use `role="alert"` with retry where recovery is available. Missing telemetry is shown as unavailable or with explicit no-data copy, never converted into a successful state or a meaningful zero.
