...
- `POST /api/projects/:projectId/agent-presets/base-updates/:baseAgentRole/apply`
  ...
  - Explicitly invokes the configured supported local provider through the `planning` route and records execution type `agent_base_update`; the prompt requests raw JSON, while the parser tolerates supported surrounding text, markdown fences, and provider envelopes before requiring exactly one non-empty `instructionMarkdown` string and no other properties
  - Extraction, payload-shape, and line-preservation errors use the structured corrective retry path in the same provider session
  ...
  - Invalid roles, missing or stale notices, unsupported providers, provider failures, malformed or destructive output, and concurrent route changes fail without changing the preset instructions or advancing its stored bundled revision
...
### Navigation
- Dashboard interface language is selected under **Settings → Appearance → Display Settings → Language**. English is the default, and users can switch between English and Deutsch immediately without saving the Settings draft. The browser-local preference survives restarts, synchronizes across tabs in the same browser profile, and updates the root HTML `lang`; it is never written to backend settings. See [Dashboard Internationalization](./internationalization.md) for the user workflow, architecture, formatting rules, and verbatim-content boundary.
...
- The primary navigation honors the persisted Settings -> General experience mode. Easy shows Chat, Sprints, Browser Preview, Stats, Live, Settings/Config, and the internal Docs page. Standard adds Overview, Tasks, Agents, Nodes, Schedule, and the other balanced operational entries while retaining Sprints and Live. Expert is the default and shows the full set. This only changes primary navigation visibility: routes remain registered, Docs opens the embedded dashboard docs route, and the Browser Preview item still follows the project sprint-preview visibility settings.
...
- Stats follows the shared v2 `PageContainer` width and padding rhythm. Its page title and project context are unboxed at the shared PageHeader typography scale, followed by one compact responsive command rail for time-window and analysis-view controls.
...
- Composition mode explains provider share, token anatomy, cache behavior, source quality, purpose distribution, Git-blocker context, and low-data fallbacks. Its detailed workspace uses a token-flow canvas, proportional provider distribution, ranked purpose rows, runtime context, and a compact provider ledger backed by snapshot totals; the mode metric deck remains the only executive KPI band.
...
- Ledgers mode provides compact Task Telemetry, Sprint Telemetry, and Git Telemetry tabs. Task and sprint workbenches use an integrated summary strip and search/sort toolbar followed by continuous hairline-separated rows; provider, purpose, recency, token anatomy, shares, and duration percentiles remain readable without turning every value into a chip. Git ledgers use the same flat row primitive with churn visuals for insertions and deletions.
...
- Live task cards, Sprint gallery/ledger entries, Task cards, and Overview active-stream rows use one bright delivery workflow badge. Task-oriented surfaces keep Coding → Pull request → QA → CI → Merge → Completion. Sprint gallery and ledger badges use Planning → Coding → QA → Pull request → CI → Merge → Completion, deriving active Planning from the latest sprint planning invocation and keeping Planning first even before tasks exist. Running or paused planning remains authoritative while that request is active; failed or cancelled planning is authoritative only while the sprint remains idle. Once coding starts or the sprint completes, that downstream lifecycle wins and the Planning stage settles as complete instead of exposing a historical planning failure. The viewport-positioned interaction region uses a lightly translucent, blurred outer surface around the opaque Delivery flow card, plus a floating responsive arrow and opaque QA card when review data exists. Stage markers are vertically centered with their entries and anchor the dotted connector rail. The badge remains mounted when a refreshed Sprints execution snapshot has no CI events, using lifecycle/review state as the durable base and enriching PR/CI/Merge when evidence returns. Sprint cards suppress child-task gate churn during coding, reveal current-run final QA/CI at 100%, require explicit evidence before naming Merge, and ignore review, gate, and attention state from older sprint attempts.
- Easy navigation includes Sprints and the focused Live page. Standard includes Sprints, Schedule, and the same focused Live page. In both modes Live renders only its upper Stats, Race, and DAG workspace; task cards, invocation/timeline panels, attention controls, execution controls, and Git/CI/PR details remain Expert-only. The Git-status channel and Expert-only task-card/filter/timing projections are disabled while those details are hidden.
...
- The DB-backed Projects page uses a low-noise gallery with quiet translucent cards, fine borders, restrained decoration, and semantic color only where it communicates selection, status, progress, focus, or destructive intent. Signal marks selection, green marks running, red marks failed/destructive state, ember marks `Needs review`, and idle remains neutral; selected state also has a visible badge and `aria-pressed` semantics.
- Each project card presents name/source, status and source badge, one normalized repository-or-local-path row, branch, last run, optional setup progress, sprint/open/done counts, completion, and an always-visible action footer. Long names, locations, branches, source badges, and run text use single-line truncation within `min-w-0` boundaries and expose full non-empty values as title text.
- Card selection is available from the main card selection surface and the footer **Select project** control. Separate visible controls run **Setup project**, open **Project settings**, and **Delete project** without also selecting the card; settings explicitly selects the target before routing to `/config`.
- The card's **Delete project** control sends the existing dashboard delete request immediately and refreshes the gallery. The Settings Danger Zone retains its confirmed deletion workflow, and MCP project deletion remains explicitly approval-gated; the redesign does not change those contracts.
- `All`, `Running`, `Idle`, and `Failed` filters are a named wrapping tablist with live counts. Intervention projects remain in `All` but do not increment the other three filters. Native Tab plus Enter/Space activation, visible focus rings, `aria-selected`, and `aria-controls` preserve keyboard access.
- Initial loading announces a busy polite status and renders skeleton cards. Load failures render an assertive alert with an **Add Project** action. Collection-empty and filtered no-match states are polite status surfaces; no-match also offers **Show all projects**, and both grids retain the dashed **Add Project** card.
- The gallery grid uses `repeat(auto-fill,minmax(min(100%,320px),1fr))` with `min-w-0` boundaries. Header actions, count pills, filters, card actions, form controls, and dialog footers stack or wrap on small screens, while bounded dialog bodies scroll internally, preventing horizontal overflow and short-viewport clipping.
- Project gallery transitions use motion-safe utilities. Under reduced motion, optional color/progress/spinner motion and Add Project modal transitions stop while selected borders, status labels, focus rings, busy semantics, and live announcements remain.
...
- The Projects header exposes **New Project** and opens the shared modal with repository initialization selected. The dashed gallery **Add Project** card opens local import first but keeps Local Project, Git URL, and New Project source choices available; the load-error surface provides the same import entry point.
...
- Existing project cards expose a Project Setup Agent action for later initialization or regeneration, with Agents, Quicksprints, Preview Script, CI, Techstack, and opt-in Docs choices. Setup uses the existing background endpoint, reports start/running/completion/failure through toasts, disables duplicate setup, polls the matching invocation, and makes **Open invocation** available on the card once tracking has an invocation ID and in completion feedback. See [Project Initialization](./project-initialization.md).
...
- Sprint card and ledger action menus expose separate `Mark Completed` and `Mark QA Pass` controls. `Mark Completed` uses a runtime-aware endpoint: if a sprint run is still active, Code UX force-cancels its dispatches, provider invocations, leases, and runtime rows before persisting the sprint as completed, preventing the live run from projecting `running` back over the manual choice. `Mark QA Pass` persists a completed sprint-level QA run with outcome `pass`, reviewer `Manual QA`, and dashboard provenance, then resolves only an active sprint-scoped QA human handoff. It is disabled while sprint QA is actively reviewing and disappears once the latest sprint QA verdict is already passing.
...
- Settings -> General exposes Experience Mode before runtime setup, then Automation, System Runtime, Docker Runtime, and Onboarding. In System scope, a final legal-actions card provides **License**, which opens the canonical Code UX [LICENSE](https://github.com/codeux-ai/codeux/blob/main/LICENSE) externally, and **OS Software**, which opens a searchable, informational catalog of the open-source dependencies distributed with Code UX. Catalog entries show license identifiers and project links; the static catalog does not participate in Settings drafts, **Save Changes**, project overrides, or settings persistence. The old Inheritance Model card has been removed from General, and `Open Onboarding` remains available to reopen setup without clearing saved settings.
...
- Settings -> Appearance previews unsaved edits immediately in the active dashboard shell. Theme, accent color, motion, navigation mode, animated/static background selection, static color, uploaded background image, and pattern overlay all update before Save Changes; leaving Settings clears the preview back to the persisted effective settings. Accent presets drive semantic primary actions, active navigation, links, selections, and focus rings while provider identities, statuses, and chart colors stay stable. New installs default to the Code UX accent and pattern overlay `None`.
...
- `Settings > Agents` composes Project Markdown Mirror, Agent Routing, persistent skill storage, and self-reflection as independently scannable responsive cards. Agent Routing uses option-card controls for Manual versus Orchestrator mode, a shared multi-select roster for orchestrator-eligible coding agents, and disabled role selectors with explicit project-agent guidance while keeping built-in fallbacks for planning, coding, CI fix, merge conflict, dashboard reply, and clarification reply routes. Persistent skills expose a concise project status and dedicated manager entry separately from the per-agent attachment matrix, whose retrieval toggles explain why they are disabled and always show `Default off` or `Enabled`.
...
- The notice explains that the agent compares both base files and applies only important system-compatibility instructions while preserving the main prompt, custom instructions, and behavior. Supported JSON presentation noise is extracted before strict one-property validation, and parsing errors receive a structured same-session retry. The guarded merge cannot change agent metadata; malformed, destructive, stale, unsupported-provider, provider-failure, and route-change cases leave both the preset and bundled revision unchanged so the notice remains available for retry.
...
- Chat Threads mode exposes **Create Web App**, **Create Desktop App**, **Create Onlineshop**, **Create Portfolio**, and **Create Game** quickactions beside the composer, including empty threads, only while the selected project remains an eligible clean Code UX seed. Clicking a control posts a short visible chat message immediately, creates a thread first when needed, and does not open a confirmation dialog or switch to Invocations. The dashboard includes the active project's effective techstack in the quickaction metadata, using the assigned catalog entry or the catalog default plus stack item labels as suggestion tags. The backend re-checks initial-project eligibility, launches the catalog-mapped detached quicksprint with `Plan & Start`, and applies its experience-specific guidance only to that plan without changing saved project settings. The transcript then shows an app progress widget with app kind, sprint name, stack fields, metadata-driven stages, and suggestion tags. Users can keep typing in the same thread while planning runs; follow-up messages are queued until tasks exist, then appended to the sprint-level goal under `Additional direction from chat`.
...
- Chat page playful-agent copy is presentation-only and workplace-safe. Active reply statuses, invocation-container statuses, tool-call context lines, reasoning/thinking lines, and assistant mood asides use curated non-offensive humor keyed to the agent/provider/model/phase or transcript item. The 3D thought bubble keeps each line for at least twenty seconds and uses a 72-line agency/project-management deck during delegated work. Its seeded shuffle exhausts the deck before reshuffling, varies by runtime context, and avoids immediate repeats; other active status copy stays visible for at least five seconds. The thought-bubble phase label omits background/provider prefixes so `Planning in progress` remains the clear lead. These adjunct lines never replace the actual provider response, tool name, arguments, output, reasoning text, status metadata, or persisted conversation/invocation transcript content.
...
### Shared QA review and CI workflow states

Sprints, Tasks, and Live use the unified interactive workflow badge for persisted QA and event-backed pull-request state. Task and Live surfaces keep lifecycle copy beside it. Sprint gallery cells use the workflow badge as the single state authority, without duplicate hover-only lifecycle or merge-conflict badges; their blue sprint-ID lockup remains separate metadata. Human-owned active intervention appears in an amber `zZZ` cue 10px above the gallery cell without a red perimeter, while worker-owned conflicts stay inside workflow details. Running gallery cells use slow GSAP-driven spectral wave fields with soft Material-inspired blue, cyan, and violet depth; reduced motion preserves the same composition as a static frame.

- QA has four distinct presentations. A pass uses the green check treatment; an active review uses the signal-colored running treatment; completed `changes_requested` uses the blue pencil-line **QA edits** treatment; and terminal provider/runtime statuses such as `failed`, `errored`, or `cancelled` use the red **QA failed** treatment. Requested changes are a usable completed verdict, while provider failure means no usable verdict was produced.
...
- DAG workflow evidence is indexed in linear passes across tasks, runtime events, and attention items before node rendering. Node memoization observes task title, lifecycle status, QA review content, dependency identity, CI state, and strict human-attention changes so live updates neither rescan the full feed per task nor leave stale node text.
...
- **Route-owned refinement**: Nodes, Scheduler, Custom Dashboards, Agents, Onboarding, Projects, Knowledge, Browser Preview, and Chat keep their feature-specific behavior while sharing draft protection, confirmation, pending/retry, keyboard, focus-restoration, and reduced-motion contracts.
- **Executable acceptance**: The isolated browser suite covers representative merged paths without dispatching provider work or touching live projects. See [Executable Acceptance Contract](./interaction-patterns.md#executable-acceptance-contract) and the [Accessibility Quality Audit](./accessibility-quality-audit.md#route-refinement-acceptance).
...
- **Task Card Affordances:** Task cards visually represent task states cleanly without expensive 3D transformations. Dependency indicators pair compact task identifiers with normalized status copy and retain complete titles, raw state, and blocking meaning for assistive technology. A persistent task-labelled **Actions** trigger opens a viewport-contained grouped menu; unavailable actions show their reasons in place, enabled items support arrow/Home/End navigation, and dismissal restores trigger focus. Drag actions use explicit `cursor-grab` interactions, omit pointer-only helper chips from visible metadata, preserve screen-reader guidance, and become static and unavailable in reduced motion or optimistic-saving states.
