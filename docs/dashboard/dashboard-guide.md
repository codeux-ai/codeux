...
### Navigation
- Sidebar and dock navigation expose the primary routes in guided-tour order: Chat (`/chat`), Overview, Sprints, Tasks, Agents, Stats, Schedule (`/scheduler`), Memory, Knowledge (`/knowledge`), Browser, Files (`/files`, providing project and sprint File Browser capabilities), Live, and Settings/Config.
- Chat is an operational dashboard page, not a documentation placeholder. Shell navigation must send users directly to `/chat` in both dock and sidebar navigation modes, never to `/docs`, `/docs/chat`, external docs, or another placeholder target.
...
- Chat page is available at `/chat`, is DB-backed, and stores project conversation threads/messages in sqlite
- The `/chat` route renders both project conversation threads and invocation transcripts so operators can move between user-facing chat and execution/debug transcripts without leaving the operational dashboard.
