...
- Task board view models are built only for tasks present in the rendered board columns after filtering and list-windowing, while dependency indicators still resolve against the full task lookup so visible cards can reference dependencies outside the current view.
...
- The Tasks page keeps page composition in `dashboard/src/v2/TasksPage.tsx`, state orchestration in `dashboard/src/v2/hooks/use-tasks-page-controller.ts`, and focused task presentation in `dashboard/src/v2/components/tasks/` (`TaskScopePlaceholder`, `SprintSelector`, `SprintProgressCard`, and task cards). The controller owns route query sync, selected sprint scope, composer/edit state, optimistic task creation, and task CRUD/drag handlers while the page owns DOM refs and GSAP entry/focus effects.
...
- **Menus & Overlays**: Use explicit ARIA roles such as `menu`, `menuitem`, `listbox`, and `option`. Keyboard navigation (up/down arrows, Enter/Space, Escape) is strictly supported. Dropdowns with search filters must track active descendants on inputs, clearly read out selection transitions using polite live regions, and expose explicit empty states when filtering yields no results.
...
- **Task Board View Models:** `buildTaskBoardViewModel` derives board state before card models, then builds card models only for tasks in rendered columns. It passes one render timestamp into every card model so relative created-at labels are consistent across a board render and deterministic in tests.
