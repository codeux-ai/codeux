...
Sprints, Tasks, and Live use `SprintReviewBadge` for the latest persisted QA summary and `CiStatusBadge` for event-backed pull-request workflow state. The badges are additive to lifecycle status: requested QA changes, CI progress, and active attention never replace the card's task or sprint status.
