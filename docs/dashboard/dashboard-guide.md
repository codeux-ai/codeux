...
- **Global Search & Comboboxes**: The Global Search implements the ARIA combobox pattern with `aria-activedescendant` to keep focus on the text input while navigating results via Arrow keys. Result grouping is semantically communicated via `role="group"` and `aria-labelledby` wrappers instead of just visual styling. Search loading states, counts, and no-results feedback are centralized in a single hidden `aria-live` region to prevent double announcements. Visible placeholder text in these states is hidden from screen readers to enforce this pattern.
