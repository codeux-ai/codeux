...
- Tasks page keeps route-level state, project-local sprint scope routing, optimistic task insertion, and task mutation handlers in `useTaskBoardController`, while `TasksPage.tsx` wires focused task-board components under `dashboard/src/v2/components/tasks/` to render the sprint scope selector, filters, and Kanban lanes from the shared task-board view-model/action helpers.
