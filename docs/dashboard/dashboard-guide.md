...
- `GET /api/local-directories?path=/absolute/path`
  - Lists child directories for the local Add Project directory picker, including current, parent, root, and home paths for browser-style navigation. Access checks canonicalize existing paths and allowed roots, so platform-equivalent spellings such as macOS `/var` and `/private/var` are accepted only when they resolve inside the same allowed directory.
- `GET /api/git-status`
