...
- **Overlays & Modals**: Dismissible surfaces (dialogs, side panels, search overlays) can be closed via explicit buttons, clicking the backdrop, or pressing the Escape key. Focus is trapped within the overlay while open and restored to the trigger when closed. Mobile drawer navigation is also implemented as a proper dismissible dialog with these behaviors. When the desktop sidebar is minimized, nav items retain accessible names and current-page states without exposing decorative tooltip text to screen readers.
...
- **Forms**: All inputs must have associated labels (`<label>` or `aria-label`/`aria-labelledby`). Shared form fields must be understandable and recoverable by connecting labels, helper text, errors, required indicators, and validation state through accessible relationships (e.g., using `aria-describedby`, `aria-errormessage`, and `aria-invalid`). Form submissions must guard against duplicate events via `isSubmitting` checks. Upon validation failure, focus should be programmatically restored to the first invalid field. Dependency lists or complex toggles should visually and semantically (`aria-disabled`) expose their unselectable states (e.g., cycle prevention) rather than silently hiding options.
...
- **Form Validation & Submission**: When a form submission fails due to validation errors, an error summary with `role="alert"` should be surfaced and focus should be automatically shifted to the first invalid field (or the first radio element of an invalid group) to assist users (especially screen readers).
  - Use `aria-invalid` and dynamically inject validation messages into `aria-live` regions or link them via `aria-describedby`.
  - For segmented choice controls (like Task Status, Priority, or Executor type), use the accessible `role="radiogroup"` pattern with native `fieldset`, `legend`, and `role="radio"` elements rather than custom unmanaged buttons. Explicitly manage `aria-checked` and `tabindex` to ensure standard arrow-key navigation.
  - When submission fails due to network or logic errors, a retry action should be presented via `ActionFeedbackRegion`.
  - Also, ensure duplicate form submissions are prevented by verifying the `isSubmitting` state before processing the submit event. Dependencies should explicitly indicate if they cannot be selected due to cycle-prevention logic using `aria-disabled="true"` and explanatory `aria-label`s, rather than silently filtering them out.
