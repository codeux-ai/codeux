...
- Trend mode uses a compact KPI band, interactive usage chart, graph filter menu, minimap, screen-reader data table, and persistent active-bucket/series rail. The usage chart header exposes selected range, bucket count, resolution, active zoom, reset, and filters in one toolbar; hover, keyboard focus, slider exploration, minimap selection, and drag zoom all feed the same active-bucket state.
...
- System mode uses a compact debugging workbench organized around sprint state, invocation health, failure analysis, external API activity, and invocation records, featuring summary strips, status distribution, and invocation filters.
...
- Sprints page is project-scoped, creates sprint records in sqlite, and exposes a structured Import flyout with Markdown plus GitHub/GitLab/Jira issue import capabilities. The issue import modals support advanced filters, quick presets, conversation appends, linked-issue context import, and special remediation-task routing, while markdown export remains available for sprint round-tripping. See [Sprint Imports](./sprint-imports.md).
