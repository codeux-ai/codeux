...
### MCP Gateway
- **Session Hardening & Bearer Auth:** The Model Context Protocol (MCP) HTTP gateway implements robust session and lifecycle constraints. Code UX startup auto-generates a user-scoped bearer token in `~/.code-ux/security.json` when no explicit `MCP_HTTPS_AUTH_TOKEN` or `--mcp-https-auth-token` is provided. Non-loopback binds (`0.0.0.0`, `::`, or a LAN address) require an active bearer token; explicit tokens still take precedence over the generated user token. The current listener is HTTP, so remote HTTPS deployments should terminate TLS with a trusted reverse proxy/certificate while preserving bearer authentication.
