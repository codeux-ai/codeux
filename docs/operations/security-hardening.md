...
- **Preview Proxy Hardening:** The local preview proxy enforces clear boundaries on the local ports and destination hosts it will forward traffic towards, reducing blind SSRF proxy abuse. Dashboard-origin API proxy requests strip dashboard `Authorization`, `Cookie`, `Set-Cookie`, hop-by-hop, proxy-control, Code UX control, host, length, and encoding headers before forwarding, and normalize origin metadata to the selected loopback upstream.
