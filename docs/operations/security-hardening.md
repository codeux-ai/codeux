...
- **File-Browser Path Constraints:** The file-browser strictly enforces canonical workspace containment. Tree, file, change, and diff requests accept only workspace-relative paths and reject absolute paths, Windows drive paths, control characters, `.git` internals, traversal segments, and encoded or double-encoded traversal before runtime delegation. File content reads also resolve the target inside the Docker-mounted workspace with `realpath`, reject symlink escapes whose canonical target leaves `/workspace`, and only pass the validated target to the capped read command.
