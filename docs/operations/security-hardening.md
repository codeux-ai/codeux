...
- **Trusted-Host Enforcement:** Runtime data paths (`/api/*`, `/health`, and `/ready`) strictly validate `Host` and `X-Forwarded-Host` before routing. Header values are parsed through canonical URL/host handling and are rejected when they are malformed, contain control characters, include userinfo, contain path/query/hash data, or present comma-separated forwarded hosts. Allowed dashboard hosts are `localhost`, `127.0.0.1`, `[::1]`, and the canonical hostname configured through `DASHBOARD_HOST`. `X-Forwarded-Host` is honored only when it also canonicalizes to an allowed host and stays within the same dashboard-vs-preview boundary as the actual `Host`.
