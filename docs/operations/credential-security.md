...
Every remote repository boundary applies the same rule independently. Task and QA branch refreshes, preview and file-browser snapshots, invocation and virtual-worker workspace preparation, project setup, agent-preset pushes, pull-request queries, and new remote project creation resolve only the credential for the selected GitHub or GitLab host immediately around that operation. Because a new target project has no project scope yet, repository creation uses the credential's broker-authorized current management project; it does not invent a target scope or bypass allowlist, status, capability, and concurrent-change checks. Sanitized settings keep the legacy token fields empty; explicit environment/CLI Git authentication remains available only when no broker reference is configured. A configured malformed, missing, revoked, unavailable, insufficiently capable, or out-of-scope reference fails closed instead of falling back to those ambient credentials. Exact resolved values are removed from operation errors before they reach dashboard or MCP responses.
