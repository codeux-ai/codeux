...
At common runtime startup, before dashboard, headless, server-mode, stdio MCP, or MCP HTTP consumers are bound, the one-way migration reads legacy values from raw settings storage, creates project credentials for project/sprint overrides or explicitly allowlisted global credentials for system integrations, and rewrites the source record without the original value. Settings caches and the runtime settings context are refreshed immediately after migration. The legacy single-document database row is deleted only after its complete supported credential set has entered this handoff, including all provider keys, nested Qwen model-provider keys, Git hosts, Jira, importer token/secret pairs, transcription, synthesis, and embedding keys; non-secret settings are sanitized into the scoped model at the same time. Repeated startup is idempotent because migrated records contain references only. Environment and legacy `settings.json` hints enter through this same migration boundary instead of being copied into settings responses. Malformed records are replaced with secret-free payloads, and if the secure key provider or a valid project scope is unavailable, plaintext is scrubbed and resolution fails closed; there is no settings fallback.
...
Hosted Jules requests install only project scope, credential-reference, and consumer metadata in asynchronous request context. The HTTP request interceptor asks the broker for the current value on every request and retry, removes the authorization header from response and error objects, and never stores the value on the shared Jules client. A configured reference always takes precedence over `JULES_API_KEY`/CLI compatibility credentials and therefore cannot fall through to ambient authentication after denial, revocation, deletion, or malformed-reference failure.

Every remote repository boundary applies the same rule independently. Task and QA branch refreshes, preview and file-browser snapshots, invocation and virtual-worker workspace preparation, project setup, agent-preset pushes, pull-request queries, and project creation resolve only the credential for the selected GitHub or GitLab host immediately around that operation. New remote creation re-resolves for the provider API request, clone, initial push, and final checkout fetch, so rotation or revocation between boundaries takes effect immediately. Existing Git URL imports resolve at their clone or fetch boundary as well. Because a target project has no project scope until its checkout is prepared and its record is created, these operations use the credential's broker-authorized current management project; they do not invent a target scope or bypass allowlist, status, capability, and concurrent-change checks. Sanitized settings keep the legacy token fields empty; explicit environment/CLI Git authentication remains available only when no broker reference is configured. A configured malformed, missing, revoked, unavailable, insufficiently capable, or out-of-scope reference fails closed instead of falling back to those ambient credentials. Exact resolved values are removed from operation errors before they reach dashboard or MCP responses.

Git-provider availability reports reference-backed configuration from broker metadata only. An active, configured management credential with `read` capability is available; malformed, missing, revoked, unavailable, or insufficiently capable references report unavailable and never fall through to environment tokens. The availability response never decrypts or returns credential material.

Selected-project Git status follows the same boundary: when neither Git host reference is configured it retains environment-token and Git CLI compatibility, while any configured reference is resolved with the selected project scope and `read` capability for each status request. Broker denial never falls through to ambient authentication, and resolved values are removed from propagated status errors.

Sprint orchestration applies distinct broker consumers to origin fetch, unique branch discovery, branch preparation, and branch preflight. This preserves local-only Git behavior while ensuring every remote branch operation observes rotation and revocation independently.
