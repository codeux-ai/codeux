...
- The same remote-only workspace policy applies to fresh Docker-backed provider invocations outside planning, including task coding, QA review/follow-up, CI autofix, merge-conflict repair, dashboard/chat replies, worker inbox replies, project setup, and node-flow provider prompts. These fresh invocations materialize from explicit `origin/<branch>` refs in `REMOTE` mode. Continuation/restart flows may reuse preserved workspaces, but if Code UX must recreate a missing workspace it applies the same remote-only materialization rules.
