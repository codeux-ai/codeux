...
- Downstream jobs reuse that build artifact for npm install smoke validation, the cross-OS orchestration DAG matrix, full Playwright on Linux/macOS/Windows, and unsigned desktop release-candidate packages. After package smoke passes, release-candidate packaging becomes eligible immediately and runs beside the E2E and orchestration matrices. The CI lane keeps each matrix bounded (`08 Orchestration` at two shards, `09 E2E` at three shards, and `10 Release Candidate` at two shards), while GitHub's runner quota queues any excess work across the parallel lanes.
