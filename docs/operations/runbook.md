...
- If `Settings -> CLI Workflow -> Execution Mode` is `Docker`:
  ...
  - Docker runtime state is stored under `~/.code-ux/runtime/docker/<repo-hash>/` by default (override with `JULES_DOCKER_RUNTIME_ROOT`). Cached setup image build contexts and build locks live under that root so setup-cache images survive dashboard restarts and concurrent post-restart jobs wait on the same build instead of starting duplicate builds. Lock acquisition recreates a parent directory removed by restart cleanup and retries with bounded adaptive delay instead of spending provider guardrail attempts on an `ENOENT`.
