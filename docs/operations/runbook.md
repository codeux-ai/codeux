...
- Docker provider launches use readable container names such as `code-ux-codex-<session>` and mount provider arguments through a generated argv file instead of passing the full prompt through the host `docker run` command line. Secret-bearing provider environment variables are written to temporary `0600` env-files and supplied with `--env-file`, so `ps`/process-list inspection should show only the env-file path and not API key values. Packaged Windows Electron builds that fail with `spawn ENAMETOOLONG` during provider launch are using an older build or a non-provider launch path that still embeds a large payload in command arguments.
- Provider secret validation should confirm values are present only in protected runtime artifacts:
  - Docker command diagnostics may inspect `docker run` argv, structured logs, and dashboard-visible invocation output for provider family, invocation id, session id, env-file path, and config mount path. They must not contain raw Gemini, Codex/OpenAI, Claude/Anthropic, Qwen, OpenCode, Antigravity, Git host, or MCP auth token values.
  - The expected protected locations are the temporary `0600` provider env-file, the mounted provider argv file, and generated provider MCP/runtime config files such as Gemini settings JSON, Codex TOML, Claude MCP JSON, Qwen settings JSON, OpenCode config JSON, or Antigravity MCP JSON.
  - When debugging auth failures, verify secret presence by checking only the protected file contents in the short-lived runtime workspace, then delete copied snippets immediately. Do not paste raw key values into activity logs, sprint comments, PRs, release notes, or support notes.
  - Safe diagnostic output should show key names or placeholders only, for example `GEMINI_API_KEY=[REDACTED]`, `Authorization: Bearer [REDACTED]`, `--env-file /tmp/.../provider.env`, and `code-ux.session-id=<session>`.
