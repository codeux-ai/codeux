...
- `apply_settings_bundle` when the bundle is marked as containing secrets or has secret-shaped input
- `export_settings_bundle` when the compatibility `includeSecrets: true` request detects secret-shaped source settings; approval never makes plaintext exportable
...
Export defaults to the `system` scope. Every export is reference-only or redacted: credential references and configured-state metadata may be returned, but provider API keys, git tokens, issue-tracker tokens, bearer tokens, passwords, and login credential material never cross the MCP response boundary. The compatibility `includeSecrets: true` input retains the one-use approval handshake when secret-shaped source settings are detected, but an approved response is still redacted and never returns plaintext.

Apply accepts a `bundle` object and optional `scopes` for partial import. Project entries must include `projectId`; sprint entries must include both `projectId` and `sprintId` so sprint overrides can be normalized against the resolved project base. Any bundle marked as containing secrets, or any bundle whose payload includes secret-shaped fields, requires the same one-use approval before persistence. Applies continue through the normal settings sanitizer and return only counts and secret-free metadata; they never echo submitted values.

Bundles synchronize non-secret configuration and broker credential references, not broker credential values. Create, replace, or rotate values separately through write-only credential operations; bind or revoke credentials through their metadata-only broker operations. One-way startup migration can convert supported legacy settings plaintext into broker references on its own runtime, but export/apply is not a credential migration or backup mechanism.
