...
## Request-Scoped Agent, Invocation, And Thread Context
...
Task-coding provider runs use the project-manager MCP gateway with the selected agent identity in `X-Code-Ux-Agent` and the durable execution invocation in `X-Code-Ux-Invocation`. Code UX validates that the invocation is active, belongs to that agent and project, has the coding type, and matches the requested task scope. This authorizes dynamically selected fallback coding agents without broadening static project routing. Code UX adds only the audience-scoped `request_clarification` grant; it does not turn that agent into a project manager or expose `reply_to_clarification` or unrelated management tools. The project-manager reply route receives the complementary `reply_to_clarification` grant.

`request_clarification` persists a human-owned `worker_clarification` record in `project_attention_items`. The attention item is the durable public record and carries project, sprint, task, sprint-run, and dispatch ownership. Its versioned payload captures the task run, provider session, authenticated execution invocation and requester, deduplication key, Markdown question/answer, status, and timestamps. The authenticated invocation can derive omitted task-run and planning scope. Task-run-backed requests also append idempotent `worker_clarification_*` events, allowing `session-sync-step.ts` to reconstruct pending, answered, or settled clarification state after a restart without a second persistence path.
...
New task-backed requests are also scheduled through the automatic clarification coordinator. It waits for the coding turn to stop, applies the durable `clarification_reply` guardrail, invokes the configured Project manager with only read-only knowledge and skill-search tools, and delivers the returned answer through the same continuation service. Startup rescans pending items. Generation and delivery are suspended for paused or terminal sprint runs. The dispatch layer refuses ordinary retries while the clarification is pending and requires the exact continuation to create a new dispatch before the attention can settle. Generation, guardrail, or continuation failure leaves the human-owned attention open with failure evidence.

An agent-reported blocked outcome with no Git changes enters this same coordinator path even if the model failed to call `request_clarification`. The provider process ending normally is recorded as a completed coding turn, not a failed invocation. Local continuation creates a new observable Code UX invocation but must retain the same workspace, branch, and native CLI conversation; fresh provider-session fallback is disabled.
