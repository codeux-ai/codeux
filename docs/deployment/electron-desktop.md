...
pnpm 11 treats ignored dependency build scripts as an error. The workspace `allowBuilds` policy therefore approves only `onnxruntime-node`, whose install step prepares the native speech runtime copied into desktop packages. Keep this allowlist narrow; adding another package requires confirming that its build script is necessary for the packaged runtime and safe on every release runner.
