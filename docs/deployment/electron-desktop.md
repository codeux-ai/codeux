...
Electron Builder must also include `assets/**`. The model-catalog loader reads `assets/models-dev/catalog.json` for bundled token pricing, first from normal npm/source paths and then from Electron resource paths such as `resources/app.asar/assets`; omitting it makes desktop usage estimates fall back to no catalog price. Keep the catalog file-driven so user pricing overrides retain their normal precedence without a network dependency.
