...
Electron Builder also includes the `docs-web/**` runtime catalog in the app contents so `/docs` can fetch `/api/docs-web` from an installed desktop build. The same `docs-web` directory is part of the npm package `files` list for installed CLI/server runs, and `DocsWebCatalogService` resolves it relative to the compiled runtime root.
