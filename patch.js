...
if (!content.includes('import { StructuredProviderResponseService')) {
...
// Update PlanningAgentServiceDeps
const interfaceMatch = content.match(/export interface PlanningAgentServiceDeps \{[\s\S]*?\}/);
if (interfaceMatch) {
  let interfaceBody = interfaceMatch[0];
  if (!interfaceBody.includes('structuredProviderResponseService?:')) {
    interfaceBody = interfaceBody.replace(
      '  logger?: Logger;',
      '  structuredProviderResponseService?: StructuredProviderResponseService;\n  logger?: Logger;'
    );
    content = content.replace(interfaceMatch[0], interfaceBody);
  }
}

// Rename VirtualPlanningResult to PlanningResultContext and delete unnecessary fields
content = content.replace(
  /interface VirtualPlanningResult \{[\s\S]*?bodyMarkdown: string;\n  nativeSessionId: string \| null;\n/,
  'interface PlanningResultContext {\n'
);

// Add private property to class and update constructor
content = content.replace(
...
// Update runVirtualPlanningRequest signature
content = content.replace(
  '  }): Promise<VirtualPlanningResult> {',
  '  }): Promise<StructuredProviderResult<PlannedSprintPayload> & PlanningResultContext> {'
);

// Update runVirtualPlanningRequest execution logic
const executeProviderRegex = /    const result = await this\.providerExecutionService\.executeProvider\(\{[\s\S]*?    \}\);\n\n    if \(\!result\.ok\) \{[\s\S]*?      \},[\s\S]*?    \};\n  \}/;
...
content = content.replace(executeProviderRegex, replacement);

// Remove the obsolete runVirtualPlanningFollowUp and parseWithJsonRetry methods
...
// Update callers of runVirtualPlanningRequest -> parseWithJsonRetry
content = content.replace(
