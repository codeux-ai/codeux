...
export function registerAutomationCredentialRoutes(app: Express,deps:DashboardDependencies):void{
  ...
  app.post("/api/projects/:projectId/credentials/:credentialId/promote",asyncRoute(async(req,res)=>{res.json(await broker(deps).promote(requireTrimmedString(req.params.projectId,"projectId"),requireTrimmedString(req.params.credentialId,"credentialId"),strings((req.body as Record<string,unknown>).allowedProjectIds)));}));
  app.post("/api/projects/:projectId/credentials/:credentialId/restrict",asyncRoute(async(req,res)=>{const body=req.body as Record<string,unknown>;res.json(broker(deps).restrict(requireTrimmedString(req.params.projectId,"projectId"),requireTrimmedString(req.params.credentialId,"credentialId"),strings(body.allowedProjectIds),strings(body.capabilities)));}));
}
