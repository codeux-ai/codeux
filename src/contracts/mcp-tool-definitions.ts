...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    runtimeRoles: ["project_manager"],
    category: "orchestration",
    description: "Manage Code UX sprints. Use followup to save an unplanned draft that can be scheduled after another sprint; planning begins only when that draft is started. Direct planning returns immediately with ETA and polling guidance; get reports current or terminal planning guidance without resubmitting work. Destructive actions require approval.",
    inputSchema: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["list", "get", "create", "followup", "update", "delete", "start", "pause", "cancel", "force_cancel", "inspect_run", "import_issues", "plan"], description: "The sprint action to perform. Use followup for work that must remain unplanned until a later scheduled start." },
        projectId: { type: "string", description: "Required for list, create, followup, start, inspect_run." },
        ...
        name: { type: "string", description: "Optional sprint title for create, followup, and update." },
        title: { type: "string", description: "Alias for name. Optional for create, followup, and update." },
        goal: { type: "string", description: "Optional for create, followup, and update." },
        goalMarkdown: { type: "string", description: "Alias for goal. Optional for create, followup, and update." },
        originalPrompt: { type: "string", description: "Optional source prompt for create, followup, and update." },
  ...
  },
  {
    ...
    runtimeRoles: ["project_manager"],
    category: "agents_memory",
    description: "Govern node-flow authoring, validation, credential bindings, publication, version review, and operational runs. Legacy list/get/create/update/delete/validate/run/attach aliases remain available; publish, rollback, and delete require approval.",
    inputSchema: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["catalog", "get_node_definition", "create_draft", "patch_draft", "validate_draft", "create_custom_node", "update_custom_node", "validate_custom_node", "request_credential", "inspect_bindings", "dry_run", "publish", "compare_versions", "rollback", "run", "cancel", "retry", "inspect_run", "list_runs", "list", "get", "create", "update", "delete", "validate", "get_run", "attach_to_agent", "detach_from_agent", "attach", "detach"], description: "The governed node-flow action or compatibility alias to perform." },
        ...
        nodeType: { type: "string", description: "Node-definition type for get_node_definition." },
        nodeVersion: { type: "integer", minimum: 1, description: "Optional node-definition version." },
        nodeId: { type: "string", description: "Node id for custom-node and credential actions." },
        slot: { type: "string", description: "Credential slot for request_credential." },
        draftRevision: { type: "integer", minimum: 1, description: "Required optimistic revision for patch, publish, and rollback." },
        fromVersion: { type: "integer", minimum: 1 },
        toVersion: { type: "integer", minimum: 1 },
        version: { type: "integer", minimum: 1, description: "Historical version used by rollback." },
        publishedBy: { type: "string", description: "Publication audit actor; defaults to the MCP project manager identity." },
        sourceRevision: { type: "string", description: "Immutable custom-node source revision." },
        actor: { type: "string", description: "Custom-node audit actor." },
        invocationId: { type: "string", description: "Custom-node validation invocation id." },
        correlationId: { type: "string", description: "Custom-node validation correlation id." },
        manifest: { type: "object", additionalProperties: true, description: "Complete governed custom-node manifest for update_custom_node." },
        ...
        patch: { type: "object", additionalProperties: true, description: "Optimistic draft patch containing graph, operations, name, or description." },
        operations: { type: "array", items: { type: "object", additionalProperties: true }, description: "Typed graph patch operations." },
  ...
  {
    name: "run_attached_flow",
    runtimeRoles: ["project_manager"],
    audiences: ["project_manager", "worker"],
    category: "agents_memory",
    description: "Run one published node flow explicitly attached to the authenticated agent. The server derives agent and conversation identity and never exposes the flow graph or credential material.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        projectId: { type: "string", description: "Required for list, create, validate_revision, data_catalog, and credential-binding actions." },
        dashboardId: { type: "string", description: "Required for get, update, create_revision, validate_revision, publish_revision, archive, and credential-binding actions." },
        revisionId: { type: "string", description: "Required for validate_revision and publish_revision; optional for list_credential_slots to review an immutable revision." },
        ...
        slotId: { type: "string", description: "Declared slot identifier for bind_credential or unbind_credential." },
        credentialId: { type: "string", description: "Metadata identifier selected for bind_credential. Secret values are never accepted." },
        expectedBindingRevision: { type: "number", description: "Positive optimistic binding revision required for bind_credential and unbind_credential." },
  ...
  },
  {
    ...
    runtimeRoles: ["project_manager"],
    category: "platform",
    description: "Manage external chat provider setup definitions, verified connections, channel bindings, redacted health, and durable delivery state. Destructive or provider-side retry operations require approval confirmation.",
    inputSchema: {
      type: "object",
      properties: {
        action: {
        ...
        deliveryStatus: { type: "string", enum: ["pending", "sending", "delivered", "retryable_failure", "processed", "failed", "duplicate", "cancelled"], description: "Optional delivery status filter." },
        direction: { type: "string", enum: ["inbound", "outbound"], description: "Optional direction filter for list_deliveries." },
        deliveryId: { type: "string", description: "Required for retry_delivery and cancel_delivery." },
        limit: { type: ["number", "string"], description: "Optional delivery result limit. Defaults to 100 and is capped by the repository." },
