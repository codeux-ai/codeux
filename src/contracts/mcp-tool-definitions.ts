...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    runtimeRoles: ["project_manager"],
    category: "orchestration",
    description: "Manage Code UX scheduler entries. Used to list, create, schedule_sprint, schedule_quicksprint, schedule_chat, schedule_wakeup, schedule_node_flow, update, delete, and run_due. Entries can use an absolute scheduledFor time or an after_sprint_end anchor. Scheduled chat and wakeup entries post messages through the chat runtime when due. Scheduled node flows run through the node flow runtime when due. Deleting entries requires approval.",
    inputSchema: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["list", "create", "schedule_sprint", "schedule_quicksprint", "schedule_chat", "schedule_wakeup", "schedule_node_flow", "update", "delete", "run_due"], description: "The scheduler action to perform." },
        ...
        title: { type: "string", description: "Optional scheduler entry title. For schedule_chat and schedule_wakeup this can also become the chat thread title." },
        targetType: { type: "string", enum: ["sprint", "quicksprint", "chat", "wakeup", "node_flow"], description: "Required for generic create. schedule_* actions infer this." },
        status: { type: "string", enum: ["scheduled", "paused", "completed", "failed", "cancelled"], description: "Optional for update." },
        scheduledFor: { type: "string", description: "Required ISO date for absolute create/schedule actions. Optional for update, schedule_wakeup with delaySeconds, or after_sprint_end anchored entries." },
        delaySeconds: { type: ["number", "string"], description: "Positive relative delay in seconds for schedule_wakeup. Mutually exclusive with scheduledFor." },
        scheduleMode: { type: "string", enum: ["absolute", "after_sprint_end"], description: "Optional schedule mode. Defaults to absolute when scheduledFor is used." },
        anchorMode: { type: "string", enum: ["after_sprint_end"], description: "Alias for scheduleMode when creating or updating an anchored entry." },
        scheduleAnchor: { type: "object", additionalProperties: true, description: "Nested anchor payload. For after_sprint_end, requires mode and sourceSprintId; supports offsetMinutes." },
        ...
        offsetMinutes: { type: ["number", "string"], description: "Non-negative delay in minutes after the source sprint ends before the entry becomes due." },
        anchorOffsetMinutes: { type: ["number", "string"], description: "Alias for offsetMinutes for after_sprint_end anchored schedules." },
        ...
        wakeupTarget: { type: "object", additionalProperties: true, description: "Nested wakeup target. Requires bodyMarkdown; supports threadId, title, connectionId, sourceInvocationId, and resumeAfterInvocationCompletion." },
        ...
        bodyMarkdown: { type: "string", description: "Flattened chat or wakeup message body for schedule_chat, schedule_wakeup, create, or update." },
        threadId: { type: ["string", "null"], description: "Optional existing chat thread id for scheduled chat and wakeup messages." },
        connectionId: { type: ["string", "null"], description: "Optional chat connection id for scheduled chat and wakeup messages." },
        sourceInvocationId: { type: ["string", "null"], description: "Optional source execution invocation id for scheduled wakeups." },
        resumeAfterInvocationCompletion: { type: "boolean", description: "For scheduled wakeups, when false the current MCP invocation id is not automatically attached." },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        action: { type: "string", enum: ["list", "schedule_wakeup", "schedule_task", "cancel"], description: "The restricted agent scheduler action to perform." },
        projectId: { type: "string", description: "Required for list, schedule_wakeup, and schedule_task." },
        entryId: { type: "string", description: "Required for cancel." },
        from: { type: "string", description: "Optional ISO start for list window." },
        to: { type: "string", description: "Optional ISO end for list window." },
        scheduledFor: { type: "string", description: "Absolute ISO time for schedule_wakeup and schedule_task. Mutually exclusive with relative delays." },
        delaySeconds: { type: ["number", "string"], description: "Positive relative delay in seconds for schedule_wakeup and schedule_task." },
        delayMinutes: { type: ["number", "string"], description: "Positive relative delay in minutes for schedule_wakeup and schedule_task." },
        ...
        taskId: { type: "string", description: "Required for schedule_task." },
        provider: { type: "string", enum: ["jules", "gemini", "codex", "claude-code", "qwen-code", "opencode", "antigravity", "mockup-cli"], description: "Optional provider override for schedule_task." },
      },
      required: ["action"],
    },
  },
