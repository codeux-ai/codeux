...
export function buildHelpText(appConfig: AppConfig): string {
  ...
  const lines: string[] = [
    ...
    "  --runtime-role VALUE",
    "                    Runtime role: project_manager (default)",
    "  --headless        Start MCP-only without binding the dashboard",
    "  --no-dashboard    Alias for --headless",
    "  --server-mode     Start dashboard-free server mode with authenticated MCP HTTPS",
    "  --server          Alias for --server-mode",
    ...
    "                    Bearer token for MCP HTTPS requests; required for server mode",
    "  --help, -h        Show this help message",
    "",
    "Server mode:",
    "  --server-mode implies --headless/--no-dashboard, requires MCP HTTPS to stay enabled,",
    "  and requires a bearer token for every bind host including loopback. Legacy --headless",
    "  outside server mode keeps the existing local-development MCP HTTPS behavior.",
    "",
    "Environment Variables:",
    "  JULES_API_KEY      Jules API key",
    "  DASHBOARD_PORT     Port for the dashboard (default: 4444)",
    "  CODE_UX_SERVER_MODE",
    "                     Set true to start dashboard-free authenticated server mode",
