...
function buildJsonOutputInstructions(): string {
  return [
    ...
    "   - Domains: `projects`, `sprints`, `tasks`, `scheduler`, `settings`, `agents`, `memory`, `preview`, `telemetry`.",
    "   - For reminders, resume-later requests, check-backs after completion, or wakeups after X seconds, use domain `scheduler`, action `schedule_wakeup`, and payload `{ projectId, bodyMarkdown, delaySeconds }` or `{ projectId, bodyMarkdown, scheduledFor }`.",
...
function buildMcpNativeOutputInstructions(): string {
  return [
    ...
    "You have the `manage_scheduler` MCP tool available. Use it directly for scheduler actions.",
    "You also have the `scheduler_code_ux` MCP tool available for agent-owned follow-ups, wakeups, and task reruns.",
    "",
    "`manage_code_ux` accepts `{ domain, action, payload }` where:",
    ...
    "- **scheduler** via `manage_scheduler`: `list` (projectId, from, to), `schedule_wakeup` (projectId, bodyMarkdown, delaySeconds or scheduledFor), `schedule_chat` (projectId, bodyMarkdown, scheduledFor), `schedule_sprint` (projectId, sprintId, scheduledFor), `schedule_quicksprint` (projectId, templateId, scheduledFor), `update` (entryId, ...), `delete` (entryId), `run_due` (now)",
    "",
    "**Important rules:**",
    "- Call the tool directly when the user requests a management action.",
    "- For reminders, resume-later requests, check-backs after completion, or wakeups after X seconds, call `manage_scheduler` with `action: \"schedule_wakeup\"`, `projectId`, `bodyMarkdown`, and either `delaySeconds` or `scheduledFor`.",
    "- Wakeups scheduled from a native MCP reply are automatically associated with the current invocation. Use them for end-of-task delayed follow-ups; they will not fire until the originating invocation has completed.",
