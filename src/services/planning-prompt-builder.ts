...
export function buildPlanPrompt(args: PlanPromptArgs): string {
  ...
  const parts = [
    ...
    "- Use the task count justified by repository ownership boundaries, integration seams, and risk; never compress the plan into an arbitrary numerical target.",
    "- Small localized changes may need fewer than 3 tasks; broad, security-sensitive, persistent, distributed, or cross-client changes may need more than 8.",
    ...
    "- Trace changed contracts through every relevant producer, consumer, dependency-injection/registration path, entrypoint, UI/API surface, and documentation surface.",
    "- Assign relevant authorization, validation-failure, timeout/error, retry, concurrency, idempotency, migration, restart/reconnect, stale-state, and cleanup behavior to concrete tasks.",
    "- Add a dependent fan-in integration task when multiple task branches must be exercised together; it must own executable integration/E2E coverage or concrete runtime wiring, never review-only work.",
    "- Give every sprint requirement an observable acceptance signal and do not rely on sprint-completion QA to discover obvious missing consumers, wiring, lifecycle paths, or tests.",
    "",
    "## Output Rules",
    "- Return JSON only.",
    ...
    "}",
    "",
    "The examples below illustrate the JSON schema and dependency syntax only. Their task counts are not recommendations.",
    "",
    "## Example Output A",
    "{",
