...
export class CustomNodeEgressBroker {
  ...
  private async respond(socket: Socket, payload: string): Promise<void> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Custom node HTTP request failed.";
      bridgeResponse = { ok: false, error: message.split(this.token).join("[REDACTED]") };
    }
