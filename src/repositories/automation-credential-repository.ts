...
import type {
  AutomationCredentialAccessEvent,
  AutomationCredentialBinding,
  AutomationCredentialMetadata,
  AutomationCredentialRotation,
...
interface CredentialRow {
  ...
  management_project_id: string | null;
  ...
  validation_status: AutomationCredentialMetadata["validationStatus"];
  created_at: string;
  updated_at: string;
  configured?: number;
}
...
export class AutomationCredentialRepository {
  ...
  list(projectId: string): AutomationCredentialMetadata[] {
    ...
    const rows = this.db.prepare(`
      SELECT c.*, EXISTS(
        SELECT 1 FROM automation_credential_secrets s WHERE s.credential_id = c.id
      ) AS configured
      FROM automation_credentials c
      WHERE c.project_id = ? OR (
        c.scope = 'global'
        AND EXISTS (SELECT 1 FROM json_each(c.allowed_project_ids_json) WHERE value = ?)
      )
      ORDER BY c.updated_at DESC
    `).all(projectId, projectId) as CredentialRow[];
  ...
  createWithEnvelope(input: {
    ...
    allowedProjectIds: string[];
    capabilities: string[];
  }, envelope: StoredSecretEnvelope): AutomationCredentialMetadata {
  ...
  updateMetadata(input: { credentialId: string; expectedVersion: number; name: string }): AutomationCredentialMetadata {
    const result = this.db.prepare(`
      UPDATE automation_credentials
      SET name = ?, version = version + 1, updated_at = ?
      WHERE id = ? AND version = ?
    `).run(input.name, new Date().toISOString(), input.credentialId, input.expectedVersion);
    this.requireCasUpdate(result.changes, input.credentialId, "metadata");
  ...
  replaceEnvelope(input: {
    ...
    expectedStatus: AutomationCredentialStatus;
    ...
    return this.db.transaction(() => {
      ...
      const update = this.db.prepare(`
        UPDATE automation_credentials
        SET key_id = ?, key_version = ?, version = ?, status = 'active',
            validation_status = 'untested', last_validated_at = NULL, updated_at = ?
      ...
      this.requireCasUpdate(update.changes, input.credentialId, "secret value");
  ...
  updateValidation(input: {
    credentialId: string;
    expectedVersion: number;
    status: AutomationCredentialMetadata["validationStatus"];
  }): AutomationCredentialMetadata {
    const now = new Date().toISOString();
    const result = this.db.prepare(`
      UPDATE automation_credentials
      SET validation_status = ?, last_validated_at = ?, version = version + 1, updated_at = ?
      WHERE id = ? AND version = ?
    `).run(input.status, now, now, input.credentialId, input.expectedVersion);
    this.requireCasUpdate(result.changes, input.credentialId, "validation status");
    return this.get(input.credentialId)!;
  }
  ...
  restrict(input: {
    ...
    const result = this.db.prepare(`
      UPDATE automation_credentials
      SET allowed_project_ids_json = ?, capabilities_json = ?, version = version + 1, updated_at = ?
      WHERE id = ? AND version = ?
    `).run(
      JSON.stringify(input.allowedProjectIds),
      JSON.stringify(input.capabilities),
    ...
    this.requireCasUpdate(result.changes, input.credentialId, "policy restrictions");
    return this.get(input.credentialId)!;
  }

  promoteWithEnvelope(input: {
    credentialId: string;
    managementProjectId: string;
    expectedVersion: number;
    expectedStatus: AutomationCredentialStatus;
    ...
    return this.db.transaction(() => {
      const update = this.db.prepare(`
        UPDATE automation_credentials
        SET scope = 'global', project_id = NULL, allowed_project_ids_json = ?,
            key_id = ?, key_version = ?, version = version + 1, updated_at = ?
        WHERE id = ? AND scope = 'project' AND project_id = ?
          AND management_project_id = ? AND version = ? AND status = ?
      `).run(
        JSON.stringify(input.allowedProjectIds),
        input.envelope.keyId,
        input.envelope.keyVersion,
      ...
      this.requireCasUpdate(update.changes, input.credentialId, "scope promotion");
      this.putEnvelope(input.envelope);
      return this.get(input.credentialId)!;
    });
  }

  revoke(input: { credentialId: string; expectedVersion: number }): AutomationCredentialMetadata {
    return this.db.transaction(() => {
      const current = this.get(input.credentialId);
      if (!current) throw new EntityNotFoundError(`Credential not found: ${input.credentialId}`);
      if (current.version !== input.expectedVersion) {
        throw this.concurrentModification("revocation");
      }
      ...
      `).run(new Date().toISOString(), input.credentialId, input.expectedVersion);
      this.requireCasUpdate(result.changes, input.credentialId, "revocation");
      return this.get(input.credentialId)!;
    });
  }
  ...
  private putEnvelope(envelope: StoredSecretEnvelope): void {
    this.db.prepare(`
      INSERT INTO automation_credential_secrets (
        credential_id, ciphertext, nonce, auth_tag, wrapped_data_key,
        wrap_nonce, wrap_auth_tag, key_id, key_version, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  getEnvelope(credentialId: string): StoredSecretEnvelope | null {
    const row = this.db.prepare("SELECT * FROM automation_credential_secrets WHERE credential_id = ?").get(credentialId) as SecretRow | undefined;
    return row ? {
  ...
  bind(credentialId: string, projectId: string, bindingKey: string, requiredCapabilities: string[]): AutomationCredentialBinding {
    ...
    this.db.prepare(`
      ...
      ON CONFLICT(project_id, binding_key) DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `).run(id, credentialId, projectId, bindingKey, JSON.stringify(requiredCapabilities), now, now);
    return this.getBinding(projectId, bindingKey)!;
  }

  getBinding(projectId: string, bindingKey: string): AutomationCredentialBinding | null {
    const row = this.db.prepare("SELECT * FROM automation_credential_bindings WHERE project_id = ? AND binding_key = ?").get(projectId, bindingKey) as BindingRow | undefined;
    return row ? this.mapBinding(row) : null;
  }

  recordAccess(input: Omit<AutomationCredentialAccessEvent, "id" | "createdAt">): void {
    this.db.prepare(`
      INSERT INTO automation_credential_access_events (
        id, credential_id, project_id, binding_key, capability, operation, outcome, reason, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      randomUUID(),
      input.credentialId,
  ...
  recordRotation(input: Omit<AutomationCredentialRotation, "id" | "rotatedAt">): void {
    this.db.prepare(`
      INSERT INTO automation_credential_rotations (
        id, credential_id, from_version, to_version, key_id, key_version, rotated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      randomUUID(),
      input.credentialId,
  ...
  private requireCasUpdate(changes: number, credentialId: string, operation: string): void {
    if (changes === 1) return;
    if (!this.get(credentialId)) throw new EntityNotFoundError(`Credential not found: ${credentialId}`);
    throw this.concurrentModification(operation);
  }

  private concurrentModification(operation: string): CredentialConcurrentModificationError {
    return new CredentialConcurrentModificationError(`Credential changed while ${operation} was being applied; refresh its metadata and retry with the current version.`);
  }

  private mapCredential(row: CredentialRow): AutomationCredentialMetadata {
    return {
      id: row.id,
      name: row.name,
      kind: row.kind,
      ...
      allowedProjectIds: JSON.parse(row.allowed_project_ids_json) as string[],
      capabilities: JSON.parse(row.capabilities_json) as string[],
      status: row.status,
      configured: toNumber(row.configured) === 1,
      keyId: row.key_id,
      keyVersion: toNumber(row.key_version),
  ...
  private mapBinding(row: BindingRow): AutomationCredentialBinding {
    return {
      id: row.id,
      credentialId: row.credential_id,
      projectId: row.project_id,
