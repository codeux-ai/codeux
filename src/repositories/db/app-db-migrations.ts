...
export function ensureChatProviderTables(db: DatabaseAdapter): void {
  ...
  db.exec("CREATE INDEX IF NOT EXISTS idx_chat_provider_sessions_expiry ON chat_provider_sessions (expires_at ASC) WHERE expires_at IS NOT NULL");
}
...
export function runMigrations(db: DatabaseAdapter): void {
  ...
  `);
  ensureIndex(db, "idx_memory_claims_project_status", "memory_claims", "project_id, status, updated_at DESC");
  ensureIndex(db, "idx_memory_claims_project_category", "memory_claims", "project_id, category, confidence DESC");
  ensureIndex(db, "idx_memory_claim_evidence_memory", "memory_claim_evidence", "memory_id, claim_id");
  ...
  `);
  ensureIndex(db, "idx_sprint_file_browser_sessions_project_updated", "sprint_file_browser_sessions", "project_id, updated_at DESC");
  ensureIndex(db, "idx_sprint_file_browser_sessions_sprint", "sprint_file_browser_sessions", "sprint_id, updated_at DESC");
