...
export const CustomDashboardValidationPanel: FunctionComponent<CustomDashboardValidationPanelProps> = ({
  ...
  return (
    <aside
      aria-label={translate(customDashboardMessages, "validationPanelAriaLabel")}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="font-display text-sm font-bold text-slate-900 dark:text-white">{translate(customDashboardMessages, "revisionGate")}</h2>
          <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
            {translate(customDashboardMessages, "revisionGateDescription")}
          </p>
        </div>
        <DropdownMenu
          ...
          menuAriaLabel={translate(customDashboardMessages, "selectRevisionAriaLabel")}
          content={(
            <div className="flex min-w-[14rem] flex-col gap-1 rounded-[1rem] border border-black/[0.08] bg-white/95 p-1.5 shadow-xl dark:border-white/[0.08] dark:bg-void-800/95">
              {revisions.map((revision) => (
                ...
                >
                  {translate(customDashboardMessages, "revisionWithStatus", {
                    number: revision.revisionNumber,
                    status: getRevisionValidationLabel(revision.validationStatus, locale),
                  })}
                </DropdownMenuItem>
              ))}
            </div>
        ...
        >
          <button
            ...
            aria-label={translate(customDashboardMessages, "openRevisionMenu")}
            aria-haspopup="menu"
      ...
      <div className="rounded-[1rem] border border-black/[0.06] bg-slate-900/[0.03] p-3 dark:border-white/[0.06] dark:bg-white/[0.03]">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="min-w-0 flex-1 truncate text-sm font-bold text-slate-900 dark:text-white">
            {selectedRevision
              ? translate(customDashboardMessages, "revision", { number: selectedRevision.revisionNumber })
              : translate(customDashboardMessages, "noRevision")}
          </span>
          <span className="rounded-full bg-white/70 px-2 py-0.5 text-[11px] font-bold text-slate-500 ring-1 ring-black/[0.06] dark:bg-white/[0.06] dark:ring-white/[0.08]">
            {getRevisionValidationLabel(selectedRevision?.validationStatus ?? null, locale)}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-3 gap-2">
          {stages.map((stage) => (
            ...
            >
              ...
              <p className="mt-2 text-xs font-semibold">{getValidationStageStateLabel(stage.state, locale)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
        ...
        <Button
          ...
          disabledReason={translate(customDashboardMessages, "publishDisabled")}
        >
          {translate(customDashboardMessages, "publish")}
        </Button>
        <Button variant="danger" onClick={onArchive} pending={archiving} disabled={dashboard.status === "archived"}>
          {translate(customDashboardMessages, "archive")}
        </Button>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="mb-2 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">{translate(customDashboardMessages, "validationLogs")}</h3>
          </div>
        ...
        <pre
          aria-label={translate(customDashboardMessages, "validationLogsAriaLabel")}
        ...
        >
          {logs || translate(customDashboardMessages, "noValidationLogs")}
        </pre>
      </div>
      ...
      {dashboard.publishedRevisionId ? (
        <div className="flex items-center gap-2 rounded-[0.9rem] bg-status-green/[0.08] px-3 py-2 text-xs font-semibold text-status-green ring-1 ring-status-green/20">
          ...
          {translate(customDashboardMessages, "publishedRevision", {
            number: revisions.find((revision) => revision.id === dashboard.publishedRevisionId)?.revisionNumber ?? selectedRevisionId ?? "",
          })}
        </div>
      ) : null}
    </aside>
