...
export const SprintMarkdownModal: FunctionComponent<SprintMarkdownModalProps> = ({
  ...
  const handleClose = () => {
    ...
    if (cardRef.current) gsap.to(cardRef.current, { y: MODAL_MOTION.exit.yEnd, opacity: MODAL_MOTION.exit.opacityEnd, scale: MODAL_MOTION.exit.scaleEnd, filter: MODAL_MOTION.exit.filterEnd, duration: d_card, ease: MODAL_MOTION.exit.ease });
    if (backdropRef.current) gsap.to(backdropRef.current, { opacity: 0, duration: d_backdrop, delay: reducedMotion ? 0 : 0.05, onComplete: onClose });
    else onClose();
  };
