...
export function FilterStrip<T extends string>({
  ...
  const normalizedOptions = options.map((option) => {
    ...
    return {
      value: isObj ? option.value : (option as T),
      label: isObj ? option.label : (option as string),
      disabled: isObj ? Boolean(option.disabled) : false,
    };
  });
  ...
  return (
    <div className="flex gap-1 p-1 bg-black/[0.04] dark:bg-white/[0.04] rounded-xl overflow-x-auto scrollbar-hide max-w-full" role="tablist" aria-orientation="horizontal">
            ...
            className={`relative flex-none touch-target rounded-lg px-3 py-1.5 text-xs font-semibold tracking-wide transition-[background-color,border-color,color,box-shadow,transform] focus:outline-none focus-visible:ring-2 focus-visible:ring-signal-500/50 focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-55 ${
              isActive
                ...
                : option.disabled
                  ...
                  : "text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-black/[0.03] dark:hover:bg-white/[0.03]"
            }`}
            style={{
      ...
      {showClear && onClear && (
        <button
          type="button"
          onClick={onClear}
          className="ml-1 flex-none touch-target overflow-hidden rounded-lg border-l border-slate-300 px-3 py-1.5 text-xs font-semibold tracking-wide text-slate-500 transition-[background-color,border-color,color,opacity,transform] focus:outline-none focus-visible:ring-2 focus-visible:ring-signal-500/50 focus-visible:ring-offset-1 hover:bg-slate-200/50 hover:text-slate-700 dark:border-slate-700 dark:text-slate-500 dark:hover:bg-void-600/50 dark:hover:text-slate-300"
          style={{
            transitionDuration: MOTION_TOKENS.timing.fast,
            transitionTimingFunction: MOTION_TOKENS.easing.standard,
          }}
