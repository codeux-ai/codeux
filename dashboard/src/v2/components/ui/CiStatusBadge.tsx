...
export const CiStatusBadge: FunctionComponent<CiStatusBadgeProps> = ({
  ...
  const meta = STATE_PRESENTATION[presentation.state];
  ...
  return (
    <span className={`relative inline-flex max-w-full flex-col items-start ${className}`} data-ci-state={presentation.state}>
      <button
        ref={triggerRef}
        type="button"
        aria-expanded={open}
        aria-controls={detailsId}
        aria-label={`CI status: ${presentation.accessibleLabel} ${open ? "Hide" : "Show"} workflow details`}
      ...
      >
        <StatusIcon
          aria-hidden={true}
          data-ci-icon={presentation.state === "failed" ? "failure" : presentation.state}
          className={`${compact ? "h-3 w-3" : "h-3.5 w-3.5 sm:h-4 sm:w-4"} shrink-0 ${
            presentation.state === "in_progress"
              ...
              : ""
          } ${presentation.state === "failed" ? "text-status-red" : ""}`}
          strokeWidth={2.2}
        />
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-right text-[10px] font-medium ${step.state === "failed" ? "text-status-red" : "text-slate-500 dark:text-slate-400"}`}>
                    {step.statusLabel}
                    <span className="sr-only"> ({stepMeta.label})</span>
                  </span>
                </span>
              );
            })}
