...
export const ToastProvider: FunctionComponent<{ children: ComponentChildren }> = ({ children }) => {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div role="status" aria-live="polite" aria-label="Notifications" aria-relevant="additions" className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none">
      ...
      <div role="alert" aria-live="assertive" aria-label="Error notifications" aria-relevant="additions" className="fixed bottom-4 left-4 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.filter(t => t.type === 'error').map((toast) => (
          <Toast
            key={toast.id}
            {...toast}
