...
const STATUS_META: Record<FileBrowserChangeStatus, { labelKey: "statusAdded" | "statusModified" | "statusDeleted" | "statusRenamed"; glyph: string; class: string }> = {
  added: { labelKey: "statusAdded", glyph: "A", class: "bg-status-green/15 text-status-green border-status-green/30" },
  modified: { labelKey: "statusModified", glyph: "M", class: "bg-sky-500/15 text-sky-500 border-sky-500/30" },
  deleted: { labelKey: "statusDeleted", glyph: "D", class: "bg-status-red/15 text-status-red border-status-red/30" },
  renamed: { labelKey: "statusRenamed", glyph: "R", class: "bg-ember-500/15 text-ember-500 border-ember-500/30" },
};
...
export const ChangesList: FunctionComponent<ChangesListProps> = ({ files, selectedPath, onSelect, loadingPath = null }) => {
  ...
  if (files.length === 0) {
    return (
      <div class="flex h-full flex-col items-center justify-center gap-3 p-10 text-center" role="status">
        ...
        <div class="text-sm font-semibold text-slate-700 dark:text-slate-200">{translate(fileBrowserMessages, "noChangesDetected")}</div>
        <p class="max-w-xs text-xs leading-5 text-slate-500 dark:text-slate-400">
          {translate(fileBrowserMessages, "noChangesDescription")}
        </p>
      </div>
    );
  }
  ...
  return (
    <div class="flex min-w-0 h-full flex-col overflow-y-auto dashboard-scrollbar p-2" role="listbox" aria-label={translate(fileBrowserMessages, "changedFiles")}>
      <div class="sr-only" role="status" aria-live="polite" aria-atomic="true">
        {translatePlural(fileBrowserMessages, "changedFilesAvailable", files.length, { formattedCount: formatNumber(files.length) })} {selectedPath
          ? translate(fileBrowserMessages, "selectedChangeAnnouncement", { path: selectedPath })
          : translate(fileBrowserMessages, "noChangeSelectedAnnouncement")}
      </div>
      {files.map((change) => {
        const meta = STATUS_META[change.status];
        const statusLabel = translate(fileBrowserMessages, meta.labelKey);
        ...
        return (
          <button
            ...
            aria-label={translate(fileBrowserMessages, "changeAria", {
              status: statusLabel,
              path: change.path,
              additions: formatNumber(change.additions),
              deletions: formatNumber(change.deletions),
              loading: isLoading ? translate(fileBrowserMessages, "loadingDiffSuffix") : "",
            })}
            onClick={() => onSelect(change.path)}
            title={isLoading ? translate(fileBrowserMessages, "loadingDiffFor", { path: change.path }) : change.path}
          ...
          >
            ...
            <span class="flex shrink-0 items-center gap-2 font-mono text-[11px] tabular-nums">
              {change.additions > 0 && <span class="text-status-green">+{formatNumber(change.additions)}</span>}
              {change.deletions > 0 && <span class="text-status-red">−{formatNumber(change.deletions)}</span>}
              {isLoading ? (
