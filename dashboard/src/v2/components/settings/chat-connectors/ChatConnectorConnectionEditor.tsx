...
export const ChatConnectorConnectionEditor: FunctionComponent<ChatConnectorConnectionEditorProps> = (props) => {
  ...
  const verifyDisabledReason = materialEdits
    ? "Save material setup or credential changes before testing."
  ...
  const save = async (): Promise<void> => {
    ...
    if (sensitiveChange.required) {
      const approved = await confirm.requestConfirm({
        title: `Confirm ${connection.displayName} transport change`,
        body: `This save replaces ${sensitiveChange.reasons.join(" and ")}. Existing external delivery behavior may change.`,
        confirmLabel: "Save connection changes",
        tone: "warning",
      });
      if (!approved) return;
    }
  ...
  const deleteConnection = async (): Promise<void> => {
    const approved = await confirm.requestConfirm({ title: `Delete ${connection.displayName}?`, body: "This removes the connection and its channel bindings. Provider-side messages are not deleted.", confirmLabel: "Delete connection", destructive: true });
  ...
  const confirmDeliveryAction = async (delivery: ChatProviderPublicDeliveryRecord, action: "retry" | "cancel"): Promise<void> => {
    const approved = await confirm.requestConfirm({
      title: `${action === "retry" ? "Retry" : "Cancel"} delivery?`,
      body: action === "retry" ? "Retrying can create an external provider side effect if the earlier failure was ambiguous." : "Cancellation prevents future attempts but cannot recall a message already accepted by the provider.",
      confirmLabel: action === "retry" ? "Retry delivery" : "Cancel delivery",
      tone: "warning",
    });
  ...
  return (
    <section aria-label={`${connection.displayName} connection editor`} className="min-w-0 overflow-hidden rounded-[1.45rem] border border-[color:var(--border-hairline)] bg-[var(--surface-glass)] p-4 shadow-[var(--elevation-base)] sm:p-5">
      ...
      <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3"><ChatConnectorLogo providerKind={connection.providerKind} disabled={!connection.enabled} /><div className="min-w-0"><h3 className="break-words text-sm font-semibold">{connection.displayName}</h3><p className="mt-1 break-words text-xs text-slate-500">{bridge.label}. Stored credentials remain write-only.</p></div></div>
        <div className="flex min-w-0 flex-wrap gap-2"><ActionButton label="Save connection" tone="primary" onClick={() => void save()} busy={props.pendingAction === "save"} disabled={Boolean(props.pendingAction)} /><ActionButton label="Test connection" tone="success" onClick={() => void props.onVerify(connection.id)} busy={props.pendingAction === "verify"} disabled={Boolean(props.pendingAction) || Boolean(verifyDisabledReason)} disabledReason={verifyDisabledReason} /><ActionButton label="Delete connection" tone="danger" onClick={() => void deleteConnection()} disabled={Boolean(props.pendingAction)} /></div>
      </div>
      ...
      <div className="mt-5 grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="min-w-0 space-y-4">
          <div className="grid min-w-0 gap-3 md:grid-cols-2"><label className="min-w-0 text-xs font-bold text-slate-500">Display name<TextInput value={draft.displayName} onChange={(displayName) => setDraft({ ...draft, displayName })} aria-label={`${connection.displayName} display name`} /></label><label className="min-w-0 text-xs font-bold text-slate-500">Connection status<SelectInput value={draft.status} onChange={(status) => setDraft({ ...draft, status: status as typeof draft.status })} options={["draft","active","disabled","error"].map((value) => ({ value, label: value }))} aria-label={`${connection.displayName} connection status`} /></label></div>
          <div><div className="mb-2 text-xs font-bold text-slate-500">Connection mode</div><PillChoiceGroup value={draft.bridgeMode} onChange={(mode) => setDraft({ ...draft, bridgeMode: mode as typeof draft.bridgeMode, setup: createDefaultSetupForBridge(definition, mode as typeof draft.bridgeMode), secrets: {} })} options={definition.bridgeModes.map((mode) => ({ value: mode.mode, label: getBridgeModeLabel(mode.mode), hint: mode.label }))} aria-label={`${connection.displayName} bridge mode`} /></div>
          <div className="flex items-center justify-between rounded-xl bg-[var(--fill-muted)] p-3"><span className="text-xs font-semibold">Runtime enabled</span><Toggle value={draft.enabled} onChange={() => setDraft({ ...draft, enabled: !draft.enabled })} aria-label={`${connection.displayName} enabled`} /></div>
          <label className="block min-w-0 text-xs font-bold text-slate-500">Ingress URL<TextInput value={connection.ingressUrl} onChange={() => undefined} disabled mono aria-label={`${connection.displayName} ingress URL`} /></label>
          <fieldset className="min-w-0 rounded-xl border border-[color:var(--border-hairline)] p-4"><legend className="px-1 text-xs font-bold uppercase tracking-[0.14em]">Setup</legend><div className="grid min-w-0 gap-3 md:grid-cols-2">{bridge.setupFields.map((field) => <label key={field.key} className="min-w-0 text-xs font-bold text-slate-500">{field.label}{field.required ? " *" : ""}{field.type === "boolean" ? <Toggle value={draft.setup[field.key] === true} onChange={() => setDraft({ ...draft, setup: { ...draft.setup, [field.key]: draft.setup[field.key] !== true } })} aria-label={`${connection.displayName} ${field.label}`} /> : field.type === "select" ? <SelectInput value={getText(field.key)} onChange={(value) => setDraft({ ...draft, setup: { ...draft.setup, [field.key]: value } })} options={(field.options ?? []).map((value) => ({ value, label: value }))} aria-label={`${connection.displayName} ${field.label}`} /> : <TextInput value={getText(field.key)} onChange={(value) => setDraft({ ...draft, setup: { ...draft.setup, [field.key]: value } })} mono={field.type === "url" || field.type === "command"} aria-label={`${connection.displayName} ${field.label}`} />}</label>)}</div></fieldset>
          <fieldset className="min-w-0 rounded-xl border border-[color:var(--border-hairline)] p-4"><legend className="px-1 text-xs font-bold uppercase tracking-[0.14em]">Write-only credentials</legend><div className="grid min-w-0 gap-3 md:grid-cols-2">{bridge.secretFields.map((field) => { const credential = connection.credentials.find((entry) => entry.key === field.key); return <label key={field.key} className="min-w-0 text-xs font-bold text-slate-500">{field.label}{field.required ? " *" : ""}<SecretInput value={draft.secrets[field.key] ?? ""} onChange={(value) => setDraft({ ...draft, secrets: { ...draft.secrets, [field.key]: value } })} placeholder={credential?.configured ? "Stored secret remains unchanged" : "Paste secret"} helperText={credential?.configured ? "Configured. Enter a replacement only to rotate it." : "The saved value is never returned to this browser."} mono aria-label={`${connection.displayName} ${field.label}`} /></label>; })}</div></fieldset>
        </div>
      ...
      <div className="mt-6 space-y-4"><ChatConnectorDeliveryHistory connectionName={connection.displayName} deliveries={props.deliveries} error={props.deliveryError} pendingDeliveries={props.pendingDeliveries} onInspect={(id) => void props.onInspectDelivery(id)} onRetry={(delivery) => void confirmDeliveryAction(delivery, "retry")} onCancel={(delivery) => void confirmDeliveryAction(delivery, "cancel")} /><NoticePanel title="Channel, project, and thread routing">Bindings map provider-specific channel identifiers to projects and optional thread identifiers. Shared channels require an unambiguous selector.</NoticePanel><ChatConnectorBindingEditor providerKind={connection.providerKind} connectionId={connection.id} connectionName={connection.displayName} projectOptions={props.projectOptions} agentPresetOptions={props.agentPresetOptions} busy={Boolean(props.pendingAction)} onSave={async (input) => Boolean(await props.onCreateBinding(input as CreateChatProviderChannelBindingInput))} />{props.bindings.map((binding) => <ChatConnectorBindingEditor key={binding.id} providerKind={connection.providerKind} connectionId={connection.id} connectionName={connection.displayName} binding={binding} projectOptions={props.projectOptions} agentPresetOptions={props.agentPresetOptions} onSave={async (input) => Boolean(await props.onUpdateBinding(binding.id, input as UpdateChatProviderChannelBindingInput))} onDelete={() => void (async () => { const approved = await confirm.requestConfirm({ title: `Delete ${binding.externalChannelName || binding.externalChannelId} binding?`, body: "Inbound routing and future outbound delivery for this project/channel mapping will stop.", confirmLabel: "Delete binding", destructive: true }); if (approved) await props.onDeleteBinding(binding.id); })()} />)}</div>
    </section>
  );
};
