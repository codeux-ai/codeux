...
export const StatusBadge: FunctionComponent<StatusBadgeProps> = ({ status, indicator }) => {
  ...
  const interactiveClasses = " hover:bg-black/5 dark:hover:bg-white/10 hover:border-black/20 dark:hover:border-white/20";

  return (
    <span
      role="status"
      aria-label={label}
      className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold border${transitionClasses} ${colorClasses}${interactiveClasses}`}
    >
