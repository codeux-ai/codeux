...
const baseButtonClass = "inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-4 py-2 text-[11px] font-bold uppercase tracking-[0.12em] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500/45 focus-visible:ring-offset-2 focus-visible:ring-offset-[#F9F8F4] disabled:cursor-not-allowed disabled:opacity-45 dark:focus-visible:ring-offset-void-900";
...
export const ModelCard: FunctionComponent<{
  ...
  return (
    <article className={`group relative flex h-full min-h-[21rem] flex-col overflow-hidden rounded-[1.5rem] border p-5 shadow-[0_12px_34px_rgba(15,23,42,0.05)] backdrop-blur-2xl transition-all duration-200 focus-within:ring-2 focus-within:ring-signal-500/25 dark:shadow-[0_18px_44px_rgba(0,0,0,0.24)] ${
      model.active
      ...
      <div className="flex flex-1 flex-col gap-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 gap-3">
            ...
            <div className="min-w-0">
              <h3 className="text-base font-black leading-tight tracking-tight text-slate-900 dark:text-white">
                {model.displayName}
              </h3>
          ...
          <span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.14em] ${getStatusClass(model)}`}>
        ...
        <dl className="grid grid-cols-3 gap-2">
          <div className="rounded-2xl border border-black/[0.04] bg-black/[0.025] p-3 dark:border-white/[0.05] dark:bg-white/[0.03]">
            <dt className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Dim</dt>
            <dd className="mt-1 font-mono text-xs font-semibold text-slate-700 dark:text-slate-200">{model.dimension}d</dd>
          </div>
          <div className="rounded-2xl border border-black/[0.04] bg-black/[0.025] p-3 dark:border-white/[0.05] dark:bg-white/[0.03]">
            <dt className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Size</dt>
            <dd className="mt-1 font-mono text-xs font-semibold text-slate-700 dark:text-slate-200">{formatBytes(model.sizeBytes)}</dd>
          </div>
          <div className="rounded-2xl border border-black/[0.04] bg-black/[0.025] p-3 dark:border-white/[0.05] dark:bg-white/[0.03]">
            <dt className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Lang</dt>
            <dd className="mt-1 truncate font-mono text-xs font-semibold text-slate-700 dark:text-slate-200">{model.language}</dd>
          </div>
        </dl>

        <div className="min-h-[3.75rem]">
          {model.downloading && (
            <div className="rounded-2xl border border-signal-500/15 bg-signal-500/[0.06] p-3">
              ...
              <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-black/[0.06] dark:bg-white/[0.08]" role="progressbar" aria-label={`${model.displayName} download progress`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}>
          ...
          {model.active && staleCount > 0 && !model.downloading && (
            <div className="rounded-2xl border border-ember-500/20 bg-ember-500/[0.07] p-3">
              <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-ember-600 dark:text-ember-400">
          ...
          {model.error && !model.downloading && (
            <p className="rounded-2xl border border-status-red/15 bg-status-red/[0.06] p-3 text-[11px] font-medium leading-4 text-status-red">
              {model.error}
            </p>
          )}
        </div>
      ...
      <div className="mt-5 flex items-center gap-2 border-t border-black/[0.05] pt-4 dark:border-white/[0.06]">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
          ...
          {model.active && !reembedding && (
            <button type="button" onClick={onReembed}
              ...
              Re-embed{staleCount > 0 ? ` ${staleCount}` : " All"}
            </button>
          )}
          {model.active && reembedding && (
            <span className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-signal-500/20 bg-signal-500/[0.08] px-4 py-2 text-[11px] font-bold uppercase tracking-[0.12em] text-signal-700 dark:text-signal-300">
