...
const baseButtonClass = "inline-flex min-h-9 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500/45 focus-visible:ring-offset-2 focus-visible:ring-offset-[#F9F8F4] disabled:cursor-not-allowed disabled:opacity-45 dark:focus-visible:ring-offset-void-900";
...
export const ModelCard: FunctionComponent<{
  ...
  return (
    <article className={`group relative flex h-full min-h-[16.5rem] flex-col overflow-hidden rounded-[1.25rem] border p-4 shadow-[0_10px_28px_rgba(15,23,42,0.045)] backdrop-blur-2xl transition-all duration-200 focus-within:ring-2 focus-within:ring-signal-500/25 dark:shadow-[0_14px_34px_rgba(0,0,0,0.22)] ${
      model.active
      ...
      <div className="flex flex-1 flex-col gap-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 gap-2.5">
            ...
            <div className="min-w-0">
              <h3 className="break-words text-[15px] font-black leading-tight tracking-tight text-slate-900 dark:text-white">
                {model.displayName}
              </h3>
          ...
          <span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2 py-1 text-[9px] font-bold uppercase tracking-[0.14em] ${getStatusClass(model)}`}>
        ...
        <dl className="grid grid-cols-3 gap-1.5">
          <div className="min-w-0 rounded-xl border border-black/[0.04] bg-black/[0.025] p-2.5 dark:border-white/[0.05] dark:bg-white/[0.03]">
            <dt className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Dim</dt>
            <dd className="mt-1 truncate font-mono text-[11px] font-semibold text-slate-700 dark:text-slate-200">{model.dimension}d</dd>
          </div>
          <div className="min-w-0 rounded-xl border border-black/[0.04] bg-black/[0.025] p-2.5 dark:border-white/[0.05] dark:bg-white/[0.03]">
            <dt className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Size</dt>
            <dd className="mt-1 truncate font-mono text-[11px] font-semibold text-slate-700 dark:text-slate-200">{formatBytes(model.sizeBytes)}</dd>
          </div>
          <div className="min-w-0 rounded-xl border border-black/[0.04] bg-black/[0.025] p-2.5 dark:border-white/[0.05] dark:bg-white/[0.03]">
            <dt className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Lang</dt>
            <dd className="mt-1 truncate font-mono text-[11px] font-semibold text-slate-700 dark:text-slate-200">{model.language}</dd>
          </div>
        </dl>

        <div className="min-h-[2.75rem]">
          {model.downloading && (
            <div className="rounded-xl border border-signal-500/15 bg-signal-500/[0.06] p-2.5">
              ...
              <div className="mt-2.5 h-1.5 w-full overflow-hidden rounded-full bg-black/[0.06] dark:bg-white/[0.08]" role="progressbar" aria-label={`${model.displayName} download progress`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}>
          ...
          {model.active && staleCount > 0 && !model.downloading && (
            <div className="rounded-xl border border-ember-500/20 bg-ember-500/[0.07] p-2.5">
          ...
          {model.error && !model.downloading && (
            <p className="rounded-xl border border-status-red/15 bg-status-red/[0.06] p-2.5 text-[11px] font-medium leading-4 text-status-red">
              {model.error}
            </p>
          )}
        </div>
      ...
      <div className="mt-4 flex items-center gap-2 border-t border-black/[0.05] pt-3 dark:border-white/[0.06]">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
          ...
          {model.active && reembedding && (
            <span className="inline-flex min-h-9 items-center gap-1.5 whitespace-nowrap rounded-lg border border-signal-500/20 bg-signal-500/[0.08] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-signal-700 dark:text-signal-300">
