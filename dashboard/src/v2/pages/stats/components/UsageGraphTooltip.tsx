...
export const UsageGraphTooltip: FunctionComponent<UsageGraphTooltipProps> = ({
  ...
  return (
    <div
      ...
      className="pointer-events-none absolute top-3 z-50 w-64 max-w-[calc(100vw-2rem)] -translate-x-1/2 text-wrap break-words rounded-[1.25rem] border border-[var(--stats-card-border)] border-t-2 border-t-amber-500/60 bg-[var(--stats-panel-bg)] px-5 py-4 shadow-[var(--stats-floating-shadow)] backdrop-blur-sm transition-all duration-200"
      style={{ left: `${Math.min(92, Math.max(8, left))}%` }}
    >
      <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--stats-label-color)]">{label}</div>
