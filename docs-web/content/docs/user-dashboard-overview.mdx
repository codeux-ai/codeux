...
| `/custom-dashboards` | [Custom Dashboards](/docs/user-dashboard-custom-dashboards) | Agent-generated project dashboards with detached validation and gated publication; hidden when `VITE_CODEUX_FEATURE_CUSTOM_DASHBOARDS` is disabled |
| `/config` | [Settings](/docs/user-dashboard-settings) | System / project / sprint settings hierarchy |
| `/config?category=guidance#guidance` | [Styleguides and Tech Stacks](/docs/user-dashboard-styleguides-and-tech-stacks) | Tech-stack guidance, styleguide catalogs, and custom worker instructions |
...
Shared confirmation, dirty-draft, pending/retry, keyboard, focus-restoration, and reduced-motion behavior is documented in [Dashboard Interaction Patterns](/docs/user-dashboard-interaction-patterns). Contributors and reviewers should also use the [Dashboard Accessibility Quality Audit](/docs/user-dashboard-accessibility-quality-audit).
