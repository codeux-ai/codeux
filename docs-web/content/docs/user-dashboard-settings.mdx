...
- [Dashboard Onboarding](/docs/settings-subcategories-onboarding)
...
**What it controls:** Cards show connection state, active/configured status, auth hints, and management entry points; host hints can import detected local settings. Notion, Asana, and Linear live in PM, while Miro, Lucid/Lucidspark, Figma/FigJam, and Mural live in CANVAS.

**Recommended defaults:** Configure provider and importer credentials at system scope and use project overrides only for repository-specific git hosts or project-specific importer defaults.

**Risks and gotchas:** Imported hints can reveal local auth paths; broad importer tokens can expose external workspaces to search, so verify before saving shared configuration.
...
<a id="importer-configuration"></a>

Configures read-only external work imports for Notion, Asana, Linear, Miro, Lucid/Lucidspark, Figma/FigJam, and Mural.

**What it controls:** Enablement, API token, optional secret, base URL, provider-specific default workspace/team/project/database/board/document/file identifiers, and search limit decide when each importer is active.

**Recommended defaults:** Store shared credentials at system scope. Use project overrides only when one project needs different default IDs or a different read-only token.

**Risks and gotchas:** These importers attach external context to sprints but do not write back to the provider. Use read-only or least-privilege tokens where the provider supports them.
