...
The **Settings** page (`/config`) is the unified configuration surface. It starts with a rich category overview that exposes important live values while moving expert controls into focused inline workspaces.
...
On desktop, categories stay visible in the left rail and the sticky command surface repeats the active category name and purpose beside scope and save controls. On smaller screens, the command surface shows one compact current-category button instead of the full rail. Open it to use the same Smart Find-filtered category list and match previews in a drawer. Arrow keys move between categories, Enter or Space selects one, and Escape closes the drawer and restores focus to the category button.
...
Each category opens one or more subsystem cards. Cards show a purpose summary and up to three important current values; a small number of safe primary choices can be edited directly. **Configure** replaces the overview with that subsystem's full-width control workspace, and **Back to category overview** returns without changing the draft. Inputs are typed (text, number with min/max, toggle, multi-select) and validate inline.

Local AI Runtime remains the reference card: speech input, speech output, and memory embeddings are visible at a glance. **Configure speech** and **Manage local models** now open inline workspaces within AI Models rather than modal overlays.
