...
For Jules-managed tasks, the gate claims a durable single-flight repair checkpoint before sending the session message and records the guardrail attempt once after delivery. The checkpoint survives runtime restarts and remains active until both the PR head SHA changes and the resumed Jules session reaches a terminal state. Repeated cycles therefore cannot resend one failed run, an intermediate push cannot race a still-working session, and a completed session without a published commit cannot drain the remaining attempts. Clarification and other action-required states are non-terminal and stay inside the same checkpoint while the normal action-required automation responds. Branch fallback runs are filtered by the current PR head SHA so stale CI from the previous commit is ignored.
