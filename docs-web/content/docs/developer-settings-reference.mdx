...
This scoped block identifies an already user-linked Google Drive directory on the host and follows the normal System → Project → Sprint cascade without storing API credentials. Docker runs validate that the resolved source exists and is a directory before exposing it at `/mnt/code-ux/google-drive` with the configured access mode. Host runs and invalid sources receive no mount. The agent notice refers only to this fixed container path, distinguishes the Drive from the Git workspace, and appears at most once across retries and resumed prompts.
