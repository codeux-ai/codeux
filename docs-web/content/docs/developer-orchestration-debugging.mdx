...
The automatic Playwright lane runs through `09 E2E` in `.github/workflows/ci.yml`. It shards by OS and purpose project (`navigation`, `settings`, `projects`, `tasks`, `agents`, and `config`) and relies on the `playwright.config.ts` `testMatch` globs so new specs under `tests/e2e/<purpose>/` are included without one-off commands.
