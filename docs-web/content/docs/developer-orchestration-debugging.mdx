...
Feature-branch patch and merge publication uses an expected-old-tip ref update. If a CI-fix materialization and task merge overlap, inspect the feature reflog: the losing writer must reapply its tree on the new tip and retry. A successful task merge followed by an unrelated feature-ref rewrite indicates an unsafe unconditional publication path, not a missing HOST dependency checkout.
