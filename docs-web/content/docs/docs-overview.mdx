...
| --- | --- |
| **A user** running sprints from the dashboard or an MCP client | [User Guide →](/docs/user-overview) |
| **A developer** integrating with the MCP server, HTTP API, or realtime protocol | [Developer Reference →](/docs/developer-overview) |
| **An architect or contributor** working on the engine itself | [Architecture →](/docs/architecture-overview) |
