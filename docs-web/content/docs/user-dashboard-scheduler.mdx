...
Active agent-created wakeups and task runs also appear outside the Scheduler page: the top bar shows
a scheduled-agent count for the selected project, and focusing or hovering it reveals the target
type, title, scheduled time or anchor, target summary, and status. The same entries are projected
into Notifications with stable scheduler-entry IDs, so read and dismissed state survives refreshes.
