...
QA fix handoffs are durable as well. Code UX records a requested-fix handoff before invoking the task's coding session. If restart occurs in that gap, the watch loop resumes the pending handoff; if the coding follow-up finished before its final QA update was written, the loop recognizes that execution and proceeds to verification instead of repeating the fix or leaving the task at `QA_PENDING`. When Codex cannot resume because the persisted native thread has no rollout, the self-contained QA fix handoff keeps the task workspace and retries once in a fresh Codex conversation. Strict flows that require the exact provider conversation remain fail-closed.
