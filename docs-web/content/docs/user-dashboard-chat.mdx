...
When a text-to-speech model or API is active under **Settings -> AI Models**, 3D Chat reads each new Project Manager reply aloud once. A compact control dock beneath the avatar identity holds the microphone and agent mute/unmute buttons, outside the composer. Voice defaults on, shows synthesis activity, and can be muted immediately. The preference is remembered per project in the current browser; refreshing, opening an existing thread, or changing threads does not replay historical messages. A small replay button on assistant prose messages lets you explicitly hear a message again in 3D Chat, Threads, or Invocations. Threads and Invocations never start speech automatically.
