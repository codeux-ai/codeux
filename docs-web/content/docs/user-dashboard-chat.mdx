...
3. Continue the conversation in the same provider session with a smaller context when one is active; otherwise keep the thread marked for replay from the stored handoff summary on the next turn.
