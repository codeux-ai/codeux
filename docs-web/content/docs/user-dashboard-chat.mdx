...
Each post triggers a routed invocation: the dashboard records the request, dispatches it to the chosen provider via the worker assignment service (routed through the `dashboard_reply` invocation type), and streams the reply back into the thread. The thread transcript updates from the server's returned chat message immediately, while the **Invocations** rail waits for persisted server invocation records and realtime refreshes before showing a row.
