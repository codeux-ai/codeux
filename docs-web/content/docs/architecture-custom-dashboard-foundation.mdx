...
The viewer remains isolated in a sandboxed `srcdoc` iframe. Published requests cross a frame-source, opaque-origin, per-frame-session message boundary, while validation requests carry their owning validation session. Both reach the server runtime gateway, which authorizes the revision and declared source/route before built-in access or external egress. Credentials resolve only inside the broker callback and never enter bridge payloads.
