...
Provider-launched MCP clients also send `X-Code-Ux-Agent` and `X-Code-Ux-Invocation`. The server resolves both headers into request-scoped context and validates the execution invocation before granting the narrow worker clarification audience. This binds a clarification to the exact project, task run, dispatch, and provider turn rather than trusting payload-supplied identity.
...
router
  ...
  .register("request_clarification", h.handleRequestClarification)
  .register("reply_to_clarification", h.handleReplyToClarification);
```
...
Task-coding invocations have a second narrow exception: an invocation-bound coding agent may call only `request_clarification` for its own derived runtime scope. It cannot inherit project-manager tools or call `reply_to_clarification`.
...
Worker clarification requests create a durable attention item and execution-invocation lineage. After the source coding turn settles, the clarification coordinator asks the configured Project manager for an answer and sends it through the ordinary reply service. Local CLI replies start a new observable Code UX invocation in the preserved workspace and resume the recorded provider-native conversation for Gemini, Codex, Claude Code, Qwen Code, OpenCode, and Antigravity. Missing native session evidence fails closed and leaves the attention item open for a human; it never silently starts a fresh provider conversation.
