...
- **Automatic rollback** is offered only for a proven isolated latest merge with no later sprint work. Code UX reverts that merge in a detached worktree, pushes a dedicated rollback branch without starting a coding invocation, opens the pull request, waits for green checks, and merges it automatically.
