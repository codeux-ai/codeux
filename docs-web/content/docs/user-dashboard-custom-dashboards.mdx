...
| `external_api` | Reads declared routes through the server source gateway. Egress policy and credential resolution remain server-side, and credential values never enter the viewer. |
