...
If no specific worker is preferred, Code UX falls back to load-balancing across capable connections, or spinning up an ephemeral virtual worker via the [Virtual worker service](/docs/architecture-virtual-workers).
...
See [Settings](/docs/user-dashboard-settings) and the [Settings reference](/docs/developer-settings-reference) for the full schema.
