...
New project creation always writes an explicit project techstack override. New local projects additionally receive `git.githubMode: LOCAL`; new remote projects do not. To set up a web or desktop app inside an existing project, use **Create Web App** or **Create Desktop App** in Chat Threads beside the composer, or the **Web App** / **Desktop App** idle quick action in 3D Chat; those prompts operate in the selected project, create a chat thread when needed, and use its current techstack setting, leaving an unassigned existing project as `None`.
