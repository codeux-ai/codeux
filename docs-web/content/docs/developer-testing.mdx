...
Playwright starts `node dist/index.js` on the E2E dashboard port (`4464` by default, override with `CODEUX_E2E_DASHBOARD_PORT`) with an isolated temporary HOME/USERPROFILE/XDG home. The server receives `CODE_UX_GIT_CONTAINER_MODE=host` so git probes stay Docker-independent, plus `CODEUX_E2E_PROVIDER_CLI_SHIM`, which points at `scripts/e2e/mock-provider-cli.mjs`; provider command specs and the HOST CLI-workflow test override only use that fake provider path when the explicit env var is present.
