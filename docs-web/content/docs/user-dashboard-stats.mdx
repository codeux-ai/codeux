...
The compact command band shows the selected project, generated snapshot time, sprint lens, time-window controls, and analysis mode controls before the mode-specific metric deck and workspace. On wide screens the window and view controls sit side by side; on narrow screens they wrap into a readable vertical flow.
