...
Navigation across the top of the workspace controls the primary analysis lens in this order: **Trend**, **Composition**, **Cost**, **Models**, **Providers**, **Ledgers**, and **System**. The selected lens is remembered independently for each project.
...
Combines the current Stats snapshot into one spend workspace without issuing another request. The summary deck shows **Total spend**, **Average per task**, **Average per sprint**, **Blended cost per million tokens**, and **Pricing coverage**.

The workspace reads from top to bottom:

- **Executive overview and spend trend** show adaptive-precision totals and rates, pricing confidence, keyboard-focusable time buckets, and an exact data table.
- **Token allocation** separates input, cached input, output, and reasoning volume. **Spend allocation** separately reconciles input, cached-input, output, and provider-reported fallback dollars.
- **Model and purpose allocation** ranks cost, token volume, shares, calls, and cost per call.
- **Task and sprint ledgers** support search and bidirectional sorting. Sprint rows use the canonical sprint projection, so reruns of one conceptual sprint remain one cost entity even though the general Sprint Telemetry ledger stays run-oriented.

Task and sprint averages include distinct entities with provider invocations. A fully covered `$0.00` task or sprint remains in the denominator; an entity with no provider usage does not. Coverage is based on invocation sources, not the dollar total: configured model pricing and valid provider-reported fallback are covered, missing usable pricing is unpriced, and older telemetry without coverage metadata is unknown. Partial totals are priced minimums. `Unpriced`, `Coverage unknown`, and `Unavailable` never mean free; only fully covered usage can legitimately display `$0.00`.
...
Cost data appears in the dedicated Cost lens and in supporting Usage Graph and Composition context, fueled by model pricing and valid provider-reported fallbacks.
- Review or override per-model input and output prices in **Settings → Model Pricing**. Confirm whether the active scope is System or Project before saving; use the narrowest scope that should own the override.
...
- Pricing changes affect Code UX estimates, not provider invoices. After saving a price, refresh Stats and confirm the coverage card moves only when the selected window contains matching model invocations; leave genuinely unknown models unset instead of entering a zero price.
...
- Cost allocation separates input, cached input, output, reasoning, and provider-reported fallback lanes with exact values and percentages. Ranked model and execution-purpose rows include spend, tokens, shares, calls, and cost per call; dense collections keep six leaders visible and reconcile the remainder in one bounded `Other` group.
...
- The Cost overview pairs total spend, task/sprint averages, per-call and per-million-token rates with pricing-coverage provenance. Its spend-over-time chart exposes every bucket through keyboard focus, exact focused detail, and a screen-reader data table; peak and selected buckets also use non-color markers.
