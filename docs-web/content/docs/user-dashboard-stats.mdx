...
Stats metric cards, chips, filters, and ledger rows use flat neutral surfaces with hairline borders and compact spacing so the analytics modes share one readable hierarchy. The top command band shows the selected project, generated snapshot time, sprint lens, time-window controls, and analysis mode controls before the mode-specific analysis begins.
...
- The graph frame, filter flyout, focused-bucket panel, tooltip, minimap, and series switches use flat neutral Stats surfaces; series colors remain reserved for chart lines and data markers.
...
### Models
Tracks specific model performance, invocation volume, latency, reliability, cache efficiency, reasoning share, pricing signals, and token throughput for each model used during the active timeframe.
