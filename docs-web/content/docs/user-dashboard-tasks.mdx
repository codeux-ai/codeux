...
When a completed task run includes a self-reflection rating, the shared rating badge shows the overall numeric score with a compact 5-star meter. Hovering the badge, or focusing it with the keyboard, opens a viewport-positioned details panel with each section rating, matching stars, numeric score, and any note captured by the worker.
