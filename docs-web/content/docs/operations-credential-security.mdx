...
Common runtime startup performs migration before dashboard, headless, server-mode, stdio MCP, or MCP HTTP consumers are bound. It migrates the complete supported legacy credential set—including all provider keys, nested Qwen model-provider keys, Git hosts, Jira, importer token/secret pairs, speech, and embedding keys—into project credentials or explicitly allowlisted global credentials, then removes the original values while preserving non-secret settings and refreshes settings caches and runtime context. The legacy database source is deleted only after those values enter the handoff. The migration is idempotent; malformed payloads and unavailable secure storage or project scope are scrubbed, and runtime resolution fails closed rather than using plaintext settings.
...
Remote repository operations resolve credentials independently at their execution boundary. Task and QA refreshes, preview and file-browser snapshots, provider and virtual-worker workspace preparation, project setup, agent-preset pushes, pull-request queries, and project creation resolve only the credential for the selected GitHub or GitLab host. New remote creation re-resolves for the provider API request, clone, initial push, and final checkout fetch; existing Git URL imports resolve at their clone or fetch boundary. Because the target project does not exist until checkout preparation completes, these operations use the credential's broker-authorized current management project and retain the normal allowlist, status, capability, and concurrent-change checks. Legacy token fields remain empty, exact resolved values are removed from returned errors, and a configured invalid or inaccessible reference fails closed rather than using ambient authentication. Environment or Git CLI authentication remains compatible only when no broker reference is configured.

Git-provider availability uses broker metadata without decrypting credential values. Active management references with `read` capability report available; malformed, missing, revoked, unavailable, or insufficiently capable references report unavailable and cannot fall through to environment tokens.
