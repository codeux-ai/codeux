...
Workflow deletes cascade to attachments, runs, and step runs. Project deletes cascade through all workflow tables. See [Node workflow persistence](/docs/architecture-node-workflow-persistence) for validation and contract details.
