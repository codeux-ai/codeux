...
Verified image, provider-tool, and browser identities are cached process-wide. Warm invocations do
not launch another inspection or provider-version container. Cheap bootstrap marker and ownership
checks run inside the provider container; if an asset was removed or recreated outside Code UX, the
first affected launch invalidates that one cache entry, repairs it through the singleflight prepare
path, and retries once. Managed launches use `--pull never`, so a missing local digest is recovered
explicitly instead of causing a surprise image pull on the invocation path.
An already-verified provider volume remains immediately usable during a background stable-channel
check, so warm invocations do not wait for registry metadata or a future staged version.
