...
Each category opens as a subsystem overview made from rich cards. Cards show their purpose plus up to three live values such as mode, provider, enabled state, safety posture, or capacity. **Configure** replaces the overview with one focused inline workspace; **Back to category overview** restores the card grid without changing the draft, inheritance, or saving.

This is intentionally not an accordion stack. Local AI Runtime remains the reference pattern: speech input, speech output, and memory embedding status stay visible, while **Configure speech** and **Manage local models** open full-width inline workspaces instead of modal overlays.

## Settings Surface Reduction Audit

These items can move out of Settings without deleting persisted configuration:

| Candidate | Why it can leave Settings | Destination |
| --- | --- | --- |
| Project id, source type, and base directory | Read-only project metadata. | Project details or workspace information. |
| License and Open Source Software | Static legal references outside the Settings draft. | About / Help. |
| Show onboarding again | Navigation command rather than configuration. | Help menu or command palette. |
| Provider host config path and generated endpoint rows | Read-only or derived from provider mode. | Context below the owning selector. |
| Generated Qwen/OpenCode config previews | Derived, masked troubleshooting output. | Provider-level **Troubleshoot** disclosure. |
| MCP effective config preview | Derived from the server fields. | **Preview generated config** disclosure. |

Automation, container runtime, QA reflection, and provider model controls can look duplicated, but currently represent different scopes or execution meanings. They should be consolidated before any entry is removed.
