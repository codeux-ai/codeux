...
- restores hosted-provider action-required epochs and successful intervention checkpoints from task-run events. Plan approvals, clarification replies, and resume nudges therefore are not resent merely because the process restarted while the provider still reported the same waiting revision. A newer remote revision starts a fresh episode even when Code UX missed the intermediate active state while offline; an unchanged clarification that exceeds its cooldown is escalated for human review.
