...
The separate browser-canvas compatibility bridge translates legacy `trigger`, `agent`, and `task` kinds into registered `input`, `set_fields`, and `provider_prompt` nodes and remaps their handles before draft creation. Import failure is reported without blocking the selected project's existing flow library.
