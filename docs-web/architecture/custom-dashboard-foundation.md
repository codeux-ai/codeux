...
- User package manifests, lockfiles, npm configuration, Vite/TypeScript/PostCSS/Tailwind configuration, undeclared files, traversal paths, raw secret literals, and unsupported source types are rejected before Docker starts. The generated package uses only fixed Preact, Vite, TypeScript, Tailwind, and Tailwind Vite integration dependencies; lifecycle scripts are disabled.
