...
- Validation derives an ephemeral server-controlled build manifest. It accepts at most 128 declared files (512 KiB per file and 2 MiB total). Modern mode accepts `.ts`, `.tsx`, and `.css`, requires TypeScript/TSX primary and route entries, imports declared CSS through the authoritative harness, and compiles Tailwind v4 with the server-owned `@tailwindcss/vite` configuration. A compatibility mode selected only from an HTML or browser-JavaScript manifest entry accepts `.html`, `.js`, `.mjs`, and `.css`, requires same-kind legacy route entries, and generates a fixed validation page and metadata-only browser bridge without accepting package or build configuration.
...
- The harness injects a read-only Code UX data bridge containing non-secret revision metadata, declared route state, and a source client bound to the validation session; credential bindings and values are excluded. Route-specific TSX modules are selected from the bridge state, including a validation preview's `route` query parameter, rather than from the proxy pathname.
...
- Runtime metadata persists the workspace path, log path, host port, container id/name, image, validation URL path, commands, and latest bounded/redacted error log excerpt. Passed modern revisions additionally persist a browser-ready bounded Vite `dist` artifact so the published viewer can render TSX-based drafts without a live validation container. Passed legacy HTML/JavaScript revisions keep using their immutable source entry through the existing sandboxed viewer, so validation artifacts containing session-specific bridge metadata are not published. Failed and unvalidated revisions never receive an executable viewer artifact.
...
- `/api/custom-dashboard-runtime/source` is the shared typed server source gateway for validation previews and active published revisions; it verifies ownership and declared source/route authorization, enforces bounded external egress policy, and resolves credential values through the server-side broker without placing them in iframe payloads
...
`custom-dashboard-router.ts` owns host normalization, query state, route selection, deep-link creation, and history updates. Browser `popstate` restores back/forward navigation. The opaque-origin iframe uses dependency-free hash history and frozen `routePath`/`navigate` bridge members; route messages pass the same frame-window, opaque-origin, and per-frame-session checks as source messages. Exact declared routes win, then a declared root, then the first declaration; route-less legacy bundles retain a synthetic `/` entry for the manifest module. The iframe chooses modules from host/bridge route state instead of its opaque `srcdoc` pathname, rejects undeclared navigation, and preserves declared hash-history restoration and route-aware share links. Unknown host deep links replace their current history entry with the fallback route so browser back/forward cannot loop on invalid state.
...
The viewer remains isolated in a sandboxed `srcdoc` iframe. Published requests cross a frame-source, opaque-origin, per-frame-session message boundary, while validation requests carry their owning validation session. Both reach `CustomDashboardRuntimeService`, which authorizes the revision and declared source/route before built-in access or external egress. A sole required credential slot is selected automatically, ambiguous required selection needs an explicit declared slot, and sources with no required slot can remain unauthenticated. Invalid selection and binding metadata are rejected before external egress. External requests must pass the bounded egress policy. Credentials resolve only inside the server-side broker callback and flow as trusted egress headers; credential values are never serialized into iframe or bridge payloads.
