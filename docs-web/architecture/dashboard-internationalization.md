...
The Tasks and Stats cost surfaces also use feature-owned catalogs. Task-board controls, lanes, dependency framing, workflow-stage presentation, and accessible announcements localize known dashboard states without rewriting task records, provider output, CI/runtime messages, or persisted status values. Cost summaries, pricing coverage and provenance, data-quality hints, provider-call and token summaries, allocation panels, entity ledgers, chart detail, dates, counts, percentages, and currency presentation follow the active locale while analytics inputs and provenance contracts remain unchanged.
