...
The boundary is intentionally strict: message bodies, prompts and quick-action request payloads, reasoning, tool names/arguments/output, invocation logs, scheduled instructions, provider errors, provider-authored runtime status values, entity names, and speech transcripts remain unchanged. Dashboard-owned invocation status enums are localized only when rendered and retain their stored values. German tests assert both the translated frame and the verbatim payload.
