...
`pnpm run check:dashboard-i18n` parses production dashboard TypeScript and TSX with the TypeScript compiler API. It reports literal JSX text, user-facing attributes, and presentation metadata outside feature message bundles. `scripts/dashboard-i18n-allowlist.json` contains only exact paths plus candidate fingerprints and rationales; adding, removing, or changing a candidate invalidates the fingerprint and forces review. Tests, generated English documentation content, protocol values, technical identifiers, and runtime or user data stay outside translation scope.

`tests/dashboard/v2/i18n-catalog-parity.test.ts` imports every feature bundle and checks English/German keys, message shapes, placeholders, plural categories, non-empty composed messages, and accidental HTML. `tests/dashboard/v2/i18n-runtime-boundary.test.tsx` verifies that translated framing and locale formatting do not rewrite provider messages, English docs Markdown, or project, sprint, task, and chat content.
