...
The `base` image is a multi-architecture `node:24-trixie-slim` development environment. The `browser` image adds pinned Playwright, Playwright MCP, and Linux browser dependencies, but contains no browser binary. Pull requests reject embedded browser/Widevine artifacts and smoke-test both targets with read-only permissions. GHCR publication, SBOM/provenance attestations, and digest signing run only when a GitHub Release is published after its tag is verified against `package.json` and `main`; dev pushes, schedules, and manual dispatches cannot publish runtime images.
