...
Hosted Jules CI repair does not use a virtual-worker attention item, but it uses an equivalent durable single-flight checkpoint. The gate claims the failed observation before delivery, charges it once, and waits for both a changed PR head SHA and a terminal resumed session before another repair can be sent. Same-run polling, restarts, intermediate pushes, and completed no-push sessions cannot consume additional attempts. A Jules clarification remains inside the same non-terminal repair attempt and is handled by the normal clarification responder. Fallback CI runs are matched to the current PR head SHA.
...
Antigravity permission reports, file views, listings, command results, and other tool events remain bounded tool telemetry rather than assistant prose. Text-mode calls use the final explicit assistant response, and worker/dashboard reply bodies are sanitized and capped before persistence or forwarding. This prevents tool envelopes from being prepended to replies or relayed into hosted provider sessions.
