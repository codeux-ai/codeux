...
QA fix handoffs are durable as well. Code UX records a requested-fix handoff before invoking the task's coding session. If restart occurs in that gap, the watch loop resumes the pending handoff; if the coding follow-up finished before its final QA update was written, the loop recognizes that execution and proceeds to verification instead of repeating the fix or leaving the task at `QA_PENDING`. Codex continuations pair the coding invocation with the task's durable workspace and read only that native thread's saved rollout. A one-time fresh conversation remains available only for self-contained QA fixes involving legacy records whose rollout is already unavailable.
