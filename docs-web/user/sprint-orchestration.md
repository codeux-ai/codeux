...
When a worker resolves a merge conflict, Code UX clears the task's stale `MERGE_CONFLICT` marker while keeping the task unmerged. The next protocol cycle consumes that clear history and retries the normal merge path instead of reopening the same attention item. That clear history is separate from merge-required suppression: suppression applies only after Git confirms the source branch has no commits ahead of the target feature branch. If the branch still has merge work, the task remains merge-required. If the consumed retry still reports `MERGE_CONFLICT`, the new marker is preserved and worker or human attention remains open. Task-level human conflict handoffs are dismissed automatically once the task marker is cleared, while main-merge and unrelated manual handoffs remain visible.
