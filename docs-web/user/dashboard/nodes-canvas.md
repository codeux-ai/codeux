...
The selected flow also loads the active project's agent presets and current flow attachments. Attaching and detaching use the governed node-flow APIs and refresh the binding list after each successful mutation. Project or flow changes clear the prior selection and discard late responses from the old scope. Loading and retryable failure states remain visible while the inspector receives only agent and attachment metadata.
...
Attached agents receive only the governed `run_attached_flow` capability for the project-owned binding. Runtime still verifies ownership, attachment identity, publication state, and credential policy and does not expose the graph, credentials, or broader node-flow management operations.
