...
Pending uses a neutral clock, running uses the signal-colored progress treatment, success uses a green check, and failure uses a red X. A sprint aggregates the newest state for each task workflow plus the final feature-to-default-branch merge workflow. Failure wins over running, running wins over pending, and pending wins over success, both for each step and for the overall badge.
