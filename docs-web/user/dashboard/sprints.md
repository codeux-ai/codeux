...
- **Issue Import** — Click **+ → Import** and choose **GitHub Issues**, **GitLab Issues**, or **Jira Issues**. You can search by text, labels, status, assignees, or exact issue keys (e.g., `#42` or `OPS-42`). Jira status filters use the active Jira project's workflow labels and include `All statuses`; if those labels cannot be loaded, the modal falls back to the broader category filters. Imported issues are attached as linked contexts, with Jira cards showing concise previews while the imported prompt context keeps the full Jira description. Jira issues can optionally be converted directly into security or quality tasks. Sprint updates that carry linked-issue metadata without body or conversation text preserve the existing linked-issue prompt text. Code UX attempts to auto-transition or auto-close linked issues when the sprint completes.
