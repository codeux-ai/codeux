...
Chat and invocation messages can also render live sprint/task reference cards. The frontend resolver is pure and project-scoped: it uses only the selected project's currently loaded sprint and task records, checks message metadata and `widget_metadata`, selected invocation context, markdown keys, and dashboard links, and returns nothing for stale ids or ambiguous task keys without a sprint context. The cards update through the existing sprint/task realtime refresh path, so a referenced task's status changes in the transcript without a separate chat polling loop.
