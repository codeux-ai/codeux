...
While that matching invocation runs, the latest exchange also shows a transient progress bubble with an explicit **In progress** label, the latest eligible assistant update, and a deduplicated tool count. Startup remains visible before the first update. This bubble is a live status projection only: it does not create or persist a thread message, and it disappears on invocation replacement, completion, failure, project or thread context changes, or loss of foreground ownership so the eventual stored reply appears only once. On mobile, the latest exchange remains inside the stage above the composer and scrolls within that bounded region, so replies and active progress appear immediately without moving the whole stage. Reduced motion preserves the same text and status cues without entrance or update animation.
