...
The thought area turns known runtime fields into compact cues for container startup, provider work, planning, QA review, completion, and errors. Current stage cues come only from running records or the selected thread's awaited reply; old completed or failed invocations are not presented as live activity. A background cue stays visible while the Project Manager remains idle, and an active Project Manager cue takes precedence while retaining a background activity count. Labels include the provider when known, and the accompanying workplace-safe quote is keyed by stable agent, provider, phase, and runtime context for five-second cycles. Reduced-motion mode keeps the status text while stopping its decorative dots.
...
**Create Web App** and **Create Desktop App** appear only for projects persisted as `new-local` or `new-remote` whose repository is still the clean Code UX seed. The project directory must be the repository root, Git must contain exactly one commit, and the root and tracked tree must contain only the generated `README.md` and `.gitignore`. The README must retain its initial `# <project name>` and `Initialized with Code UX.` content; the only effective ignore entry may be `.code-ux/` or `.code-ux`. Imported or legacy projects, setup artifacts, dirty or ignored extra content, altered seed files, additional files or commits, missing checkouts, and inspection errors all fail closed. The dashboard refreshes eligibility after project structure and Git changes, and the server verifies it again at launch, so Web/Desktop disappear after setup or any repository change.

**Create Onlineshop**, **Create Portfolio**, and **Create Game** remain available on normal project-scoped chat paths and do not require an initial repository. Invalid kind, template, or guidance metadata fails safely instead of starting a different sprint.
...
On desktop the controls use a balanced two-column grid beside the avatar; on mobile they become a horizontally scrollable two-row layout. Long labels wrap. Every control is a keyboard-reachable button with a visible focus state, and Enter or Space activates it. Reduced-motion mode stops the quickaction floating animation (along with the stage's other decorative motion) without removing actions or status text. The whole group hides without a selected project and while chat is sending, working, or showing an error; Web/Desktop also remain hidden until initial-project eligibility has loaded and passed.
...
Dashboard replies may also request a short avatar reaction through `metadata.agentEffect`. The runtime accepts only documented emotion and animation identifiers, an optional caption up to 120 characters, and an integer duration from 500 through 10000 milliseconds. JSON-mode providers emit the object beside `replyMarkdown`; MCP-native providers use a `codeux:agent` JSON fence that is validated and removed from visible markdown. The cinematic stage applies a validated effect only to the latest Project Manager reply and only for its bounded duration; errors, outgoing message routing, and active Project Manager work remain authoritative. The reply markdown, widgets, timestamp, and Threads transcript remain visible and unchanged. Invalid effects are omitted while malformed native fences are shown as ordinary JSON code blocks. Reduced-motion mode keeps the semantic emotion and caption as a static cue while skipping the requested choreography. Avatar-only effect metadata and fences are not sent to external chat channels.
