...
Chat and invocation messages can also carry live sprint/task references for future rich entity cards. The frontend resolver is pure and project-scoped: it uses only the currently loaded sprint and task records, checks message metadata and `widget_metadata`, selected invocation context, markdown keys, and dashboard links, and returns nothing for stale ids or ambiguous task keys without a sprint context.
