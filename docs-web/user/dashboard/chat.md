...
Dashboard replies may also request a short avatar reaction through `metadata.agentEffect`. The runtime accepts only documented emotion and animation identifiers, an optional caption up to 120 characters, and an integer duration from 500 through 10000 milliseconds. JSON-mode providers emit the object beside `replyMarkdown`; MCP-native providers use a `codeux:agent` JSON fence that is validated and removed from visible markdown. Invalid effects are omitted while the markdown answer and any management action continue normally. Avatar-only effect metadata and fences are not sent to external chat channels.
