...
These terminal QA states apply while the task remains unsettled. After a task is authoritatively completed, an older failed, cancelled, or changes-requested review remains available in audit history but no longer replaces **Completed** with a stale **QA edits** control. An actively running review remains visible, and a passing review can stay attached as positive completion evidence.
