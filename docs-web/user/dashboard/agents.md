...
The dashboard reply route always receives the full built-in Code UX MCP surface, scheduler, and the default Playwright MCP server for dashboard chat turns. This is keyed to the dashboard reply route assignment rather than the generic project-manager role. Enabling scheduler or any other Code UX tool for planning, coding, QA, CI repair, merge-conflict, or other non-chat agents is riskier because those agents run during operational workflows and can affect project state without being part of a direct dashboard chat exchange.
