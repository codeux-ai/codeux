...
Remote repository operations resolve credentials independently at their execution boundary. Task and QA refreshes, preview and file-browser snapshots, provider and virtual-worker workspace preparation, project setup, agent-preset pushes, pull-request queries, and new remote project creation resolve only the credential for the selected GitHub or GitLab host. Because the new target project does not exist yet, repository creation uses the credential's broker-authorized current management project and retains the normal allowlist, status, capability, and concurrent-change checks. Legacy token fields remain empty, exact resolved values are removed from returned errors, and a configured invalid or inaccessible reference fails closed rather than using ambient authentication. Environment or Git CLI authentication remains compatible only when no broker reference is configured.
