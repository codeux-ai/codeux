...
Shared subprocess execution validates command names, arguments, stdin files, and working directories immediately before spawning. Working directories must resolve to existing real directories before either the inline or helper-process boundary, and `shell: false` prevents argument values from being reinterpreted as shell syntax.
