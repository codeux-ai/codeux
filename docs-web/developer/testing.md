...
`pnpm run test:e2e` is a wrapper around `pnpm exec playwright test`; it chooses an isolated dashboard/MCP port pair and exports `CODEUX_E2E_DASHBOARD_PORT` before Playwright starts `node dist/index.js` and waits on `/ready`. The server runs with an isolated temporary HOME/USERPROFILE/XDG home and receives `CODEUX_E2E_PROVIDER_CLI_SHIM`, which points at `scripts/e2e/mock-provider-cli.mjs`; provider command specs only use that fake provider when the explicit env var is present.
