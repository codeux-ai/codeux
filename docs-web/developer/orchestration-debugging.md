...
Use this suite when a sprint stalls, local merges fail, worker-owned attention items churn, or memory usage needs extended observation after a fix. CI uses the compiled QA DAG lane by default; its Linux Docker run waits for a durable task transition before restarting the isolated runtime twice during active QA work, while macOS and Windows run the matching Electron QA DAG. If startup finds a QA row without a backing invocation, it cancels and retries that row immediately; only completed review verdicts count toward QA pass/fail history. Full mockup pentest lanes are manual escalation tools for targeted investigations.
