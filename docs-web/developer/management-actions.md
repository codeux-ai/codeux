...
| `create` | – | `projectId`, `targetType`, `scheduledFor`, target payload | Create a generic scheduler entry for `sprint`, `quicksprint`, `chat`, `wakeup`, `node_flow`, or `memory_remediation`. |
...
| `schedule_wakeup` | – | `projectId`, `bodyMarkdown`, `scheduledFor` or `delaySeconds` | Schedule a one-time chat wakeup. Optional `threadId`, `connectionId`, `title`, `timezone`, `sourceInvocationId`, `resumeAfterInvocationCompletion`. |
