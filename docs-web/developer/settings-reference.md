...
This scoped block identifies an already user-linked Google Drive directory on the host and follows the normal System → Project → Sprint cascade without storing API credentials. Paths are trimmed during sanitization, unsupported nested fields are removed, and missing or invalid access modes disable the block and fall back to read-only access. The shared provider boundary uses sprint-effective settings when a sprint is in scope and project-effective settings otherwise, validates the source once per invocation, and reuses the typed mount for retries and session continuation across Project Manager replies, planning, setup, task coding, QA, CI repair, and merge repair. Docker maps the source for the daemon before exposing it at `/mnt/code-ux/google-drive` with read-only or read-write access. Dashboard replies, thread compaction, and clarification replies use this same scoped resolution. Host runs, unscoped runs, and invalid sources receive no mount. The agent notice refers only to this container path, distinguishes the Drive from the Git workspace, and is idempotent across retries and resumed prompts.
