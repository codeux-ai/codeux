...
| Method | Path | Description |
| --- | --- | --- |
| `GET` | `/api/projects/:projectId/scheduler` | List scheduler entries and occurrences. |
| `POST` | `/api/projects/:projectId/scheduler` | Create a scheduler entry. |
| `PATCH` | `/api/scheduler/:entryId` | Update an entry. |
| `DELETE` | `/api/scheduler/:entryId` | Delete an entry. |
| `POST` | `/api/scheduler/run-due` | Evaluate and run due entries now. |
...
| Method | Path | Description |
| --- | --- | --- |
| `GET` | `/api/projects/:projectId/knowledge/documents` | List knowledge documents. |
| `POST` | `/api/projects/:projectId/knowledge/documents` | Add documents (upload, paste, or repo path). |
| `POST` | `/api/projects/:projectId/knowledge/documents/import-project` | Import knowledge from another project. |
| `POST` | `/api/projects/:projectId/knowledge/search` | Semantic search over the project's documents. |
| `GET` | `/api/knowledge/documents/:documentId` | Get a document. |
| `DELETE` | `/api/knowledge/documents/:documentId` | Delete a document. |
| `POST` | `/api/knowledge/documents/:documentId/reembed` | Re-embed a document. |
| `GET` | `/api/agent-presets/:agentPresetId/knowledge` | List documents an agent is subscribed to. |
| `PUT` | `/api/agent-presets/:agentPresetId/knowledge/subscriptions` | Set an agent's document subscriptions. |
...
| Method | Path | Description |
| --- | --- | --- |
| `GET` | `/api/local-directories?path=` | List child directories for the local Add Project picker. Existing paths and allowed roots are canonicalized before filesystem access, so equivalent host spellings such as macOS `/var` and `/private/var` resolve to the same allowed directory. |
| `GET` | `/api/local-files?path=` | List child directories and file names for trusted local file picking; file contents are not returned. Uses the same canonicalized access check as `/api/local-directories`. |
...
| `POST` | `/api/file-browser/sessions/:sessionId/stop` | Stop a session. |
| `DELETE` | `/api/file-browser/sessions/:sessionId` | Remove a session. |
| `GET` | `/api/file-browser/sessions/:sessionId/tree` | Get the file tree. |
| `GET` | `/api/file-browser/sessions/:sessionId/file` | Get a file's contents. |
| `GET` | `/api/file-browser/sessions/:sessionId/changes` | List the sprint's changed files. |
| `GET` | `/api/file-browser/sessions/:sessionId/diff` | Get a diff for a changed file. |
