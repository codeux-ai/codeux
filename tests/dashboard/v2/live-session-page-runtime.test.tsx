...
vi.mock("../../../dashboard/src/v2/hooks/use-live-session-actions.js", () => ({
  useLiveSessionActions: vi.fn((_refreshRuntimeStatus: any, _refreshGitStatus: any, requestConfirm: any) => ({
    rerunningIds: liveActionMocks.rerunningIds,
    pendingActionIds: liveActionMocks.pendingActionIds,
...
describe("LiveSessionPage Runtime Status", () => {
  beforeEach(() => {
    cleanup();
    vi.clearAllMocks();
    liveActionMocks.rerunningIds = new Set();
    liveActionMocks.pendingActionIds = new Set();
  ...
  it("keeps pending runtime actions inert with target-specific accessible names", () => {
    liveActionMocks.pendingActionIds = new Set(["sprint-pause:run-pending"]);
    vi.mocked(useDashboardRuntimeData).mockReturnValue({
      error: null,
      gitStatus: null,
      gitStatusError: null,
    ...
    expect(liveActionMocks.handlePauseSprintRun).not.toHaveBeenCalled();
  });
