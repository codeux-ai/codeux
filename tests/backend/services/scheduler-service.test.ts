...
const createWakeupEntry = (overrides: Partial<SchedulerEntryRecord> = {}): SchedulerEntryRecord => createEntry({
  targetType: "wakeup",
  sprintTarget: undefined,
  wakeupTarget: {
    bodyMarkdown: "Resume the follow-up.",
    threadId: "thread-1",
...
describe("SchedulerService", () => {
  ...
  it("posts immediate ungated wakeups through the chat runtime", async () => {
    const entry = createWakeupEntry();
    const repo = {
      listDueEntries: vi.fn(() => [entry]),
      getEntry: vi.fn(() => entry),
      markRunSucceeded: vi.fn(),
    ...
    const service = buildService(repo, { chatThreadRuntimeService });
  ...
  it("defers gated wakeups while the source invocation is still running", async () => {
    const entry = createWakeupEntry({
      wakeupTarget: {
        bodyMarkdown: "Resume after the source finishes.",
    ...
    const executionRepository = {
      getExecutionInvocation: vi.fn(() => ({ id: "invocation-1", status: "running" })),
    };
    const chatThreadRuntimeService = {
      postMessage: vi.fn().mockResolvedValue({ id: "message-1" }),
    };
    const service = buildService(repo, { executionRepository, chatThreadRuntimeService });

    await service.runDueEntries(new Date("2026-05-18T09:00:01.000Z"));

    expect(executionRepository.getExecutionInvocation).toHaveBeenCalledWith("invocation-1");
  ...
  it("posts gated wakeups after the source invocation completes", async () => {
    const entry = createWakeupEntry({
      wakeupTarget: {
        bodyMarkdown: "Resume after the source finishes.",
        threadId: "thread-1",
        title: "Invocation wakeup",
        connectionId: "connection-1",
        sourceInvocationId: "invocation-1",
      },
    });
    ...
    const executionRepository = {
      getExecutionInvocation: vi.fn(() => ({ id: "invocation-1", status: "completed" })),
    };
  ...
  it.each(["failed", "cancelled"] as const)(
    "marks gated wakeups failed when the source invocation is %s",
    async (status) => {
      const entry = createWakeupEntry({
        wakeupTarget: {
          bodyMarkdown: "Resume after the source finishes.",
      ...
      const executionRepository = {
        getExecutionInvocation: vi.fn(() => ({ id: "invocation-1", status })),
      };
      const chatThreadRuntimeService = {
        postMessage: vi.fn().mockResolvedValue({ id: "message-1" }),
      };
      ...
      expect(repo.markRunFailed).toHaveBeenCalledWith(
        "entry-1",
        `Scheduled wakeup source invocation invocation-1 ended with status ${status}.`,
      );
    },
  );
