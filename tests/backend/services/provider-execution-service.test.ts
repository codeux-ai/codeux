...
describe("ProviderExecutionService", () => {
  ...
  it("records failed provider invocation metadata and sanitized failure reason without leaking secrets", async () => {
    ...
    const failedResult = {
      ...
      nativeSessionId: "native-failed-1",
      usageTelemetry: {
        ...mockResult.usageTelemetry,
        transcriptText: "failure transcript",
        totalTokens: 31,
      },
    };
    providerRunner.runProvider.mockResolvedValue(failedResult);
    ...
    vi.mocked(classifyProviderError).mockReturnValue({
      category: "UNKNOWN",
      userMessage: "Provider failed with OPENAI_API_KEY=sk-live and authPath=/home/me/.config/qwen",
      resetAtIso: null,
      provider: "claude-code",
      resetAfter: null,
    });
    ...
    expect(executionRepository.createProviderInvocationUsage).toHaveBeenCalledWith(expect.objectContaining({
      ...
      promptChars: "test prompt".length,
    ...
    expect(executionRepository.updateExecutionInvocation).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        lastErrorMessage: "Provider failed with OPENAI_API_KEY=[REDACTED] and authPath=[REDACTED]",
        lastRetryAfterIso: null,
      }),
    );
    expect(executionRepository.appendExecutionInvocationMessage).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        role: "system",
        contentMarkdown: "Provider error (UNKNOWN): Provider failed with OPENAI_API_KEY=[REDACTED] and authPath=[REDACTED]",
        metadata: expect.objectContaining({
          provider: "claude-code",
          model: "test-model",
    ...
    const persistedMessages = executionRepository.appendExecutionInvocationMessage.mock.calls
      .map(([, message]) => JSON.stringify(message))
      .join("\n");
    expect(persistedMessages).not.toContain("sk-live");
    expect(persistedMessages).not.toContain("ghp_123456789012345678901234567890123456");
    expect(persistedMessages).not.toContain("/home/me/.config/qwen");
    expect(logger.info).toHaveBeenCalledWith("Provider invocation started", expect.objectContaining({
      logPurpose: "invocation",
      provider: "claude-code",
      model: "test-model",
      purpose: "test-purpose",
    }));
    expect(logger.info).toHaveBeenCalledWith("Provider invocation finished", expect.objectContaining({
      logPurpose: "invocation",
      provider: "claude-code",
      model: "test-model",
      ok: false,
