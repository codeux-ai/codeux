...
describe("runSessionSyncStep", () => {
  ...
  it("settles failed provider sessions without leaving a running dispatch", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "code-ux-session-sync-provider-failure-"));
    tempDirs.push(dir);
          ...
          sessions: [
            {
              id: "failed-session",
              name: "sessions/failed-session",
              title: "Sprint 6: [run:my-repo/s6/t01] [T01] Sync provider failure",
              state: "FAILED",
              provider: "codex",
            },
          ],
    ...
    expect(executionRepository.listTaskRunEvents(run.id)).toEqual([
      expect.objectContaining({
