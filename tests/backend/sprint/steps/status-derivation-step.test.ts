...
describe("runStatusDerivationStep", () => {
  ...
  it("unblocks dependent tasks when dependencies are completed with PR_CREATED merge indicator", () => {
    const subtasks: Subtask[] = [
      { id: "task-1", title: "Task 1", prompt: "", depends_on: [], is_independent: true, is_merged: false, status: "COMPLETED", merge_indicator: "PR_CREATED", pr_url: "url" },
      { id: "task-2", title: "Task 2", prompt: "", depends_on: ["task-1"], is_independent: false, is_merged: false, status: "BLOCKED" },
    ];
    const result = runStatusDerivationStep(subtasks, { retryFailed: true, isActionRequiredState });
    expect(result[1].status).toBe("PENDING");
